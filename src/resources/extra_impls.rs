...
// ---------------------------------------------------------------------------
// Streamtube cylinder mesh builder
// ---------------------------------------------------------------------------

/// Build an 8-sided cylinder mesh for streamtube rendering.
///
/// The cylinder runs from Y = −1 to Y = +1 (total length 2) with XZ radius = 1.
/// The per-instance transform in the shader scales it to `(radius, half_len, radius)`
/// and rotates it so the +Y axis aligns with the segment direction.
///
/// Returns (vertices, indices) ready for upload using the full `Vertex` layout.
/// Index count: 96 (32 triangles — 16 for the tube surface, 8+8 for caps).
pub(super) fn build_streamtube_cylinder() -> (Vec<Vertex>, Vec<u32>) {
    let sides = 8usize;
    let white = [1.0f32, 1.0, 1.0, 1.0];

    // We need separate vertices for the tube sides (outward normals) and the
    // caps (axial normals), so we duplicate the ring positions.
    //
    // Vertex layout:
    //   0..8   — bottom tube ring (y = -1, normals radially outward)
    //   8..16  — top    tube ring (y = +1, normals radially outward)
    //  16..24  — bottom cap  ring (y = -1, normals (0,-1,0))
    //  24..32  — top    cap  ring (y = +1, normals (0,+1,0))
    //  32      — bottom center    (y = -1, normal (0,-1,0))
    //  33      — top    center    (y = +1, normal (0,+1,0))
    //  Total: 34 vertices

    let mut verts: Vec<Vertex> = Vec::with_capacity(34);
    let mut indices: Vec<u32> = Vec::with_capacity(96);

    let zero_uv = [0.0f32, 0.0];
    let zero_tan = [1.0f32, 0.0, 0.0, 1.0];

    // ---- ring vertices ----
    for ring in 0..2 {
        let y = if ring == 0 { -1.0f32 } else { 1.0 };
        for s in 0..sides {
            let angle = (s as f32) * std::f32::consts::TAU / (sides as f32);
            let (sin_a, cos_a) = angle.sin_cos();
            // tube ring — outward normals
            verts.push(Vertex {
                position: [cos_a, y, sin_a],
    ...
    // cap rings (same XZ positions, axial normals)
    for ring in 0..2 {
        let y = if ring == 0 { -1.0f32 } else { 1.0 };
        let ny = if ring == 0 { -1.0f32 } else { 1.0 };
        for s in 0..sides {
            let angle = (s as f32) * std::f32::consts::TAU / (sides as f32);
            let (sin_a, cos_a) = angle.sin_cos();
            verts.push(Vertex {
    ...
    let bottom_tube = 0u32;
    let top_tube = sides as u32;
    let bottom_cap = (sides * 2) as u32;
    let top_cap = (sides * 3) as u32;
    let bot_center = (sides * 4) as u32;
    let top_center = (sides * 4 + 1) as u32;

    // ---- tube side quads: 2 triangles per quad, 8 quads = 48 indices ----
    for s in 0..sides as u32 {
        let next = (s + 1) % sides as u32;
        let b0 = bottom_tube + s;
        let b1 = bottom_tube + next;
        let t0 = top_tube + s;
        let t1 = top_tube + next;
        // CCW winding viewed from outside.
        indices.extend_from_slice(&[b0, t0, b1]);
        indices.extend_from_slice(&[b1, t0, t1]);
    }

    // ---- bottom cap (fan from bot_center): 8 triangles = 24 indices ----
    for s in 0..sides as u32 {
        let next = (s + 1) % sides as u32;
        let c0 = bottom_cap + s;
        let c1 = bottom_cap + next;
        // CCW when viewed from below (looking up +Y, normal is -Y).
        indices.extend_from_slice(&[bot_center, c1, c0]);
    }

    // ---- top cap (fan from top_center): 8 triangles = 24 indices ----
    for s in 0..sides as u32 {
        let next = (s + 1) % sides as u32;
        let c0 = top_cap + s;
        let c1 = top_cap + next;
        // CCW when viewed from above (normal +Y).
        indices.extend_from_slice(&[top_center, c0, c1]);
    }
