...
- **The point cloud pipelines are built on first draw, not on upload** - `upload_point_cloud` used to compile the render pipeline so a load-time upload absorbed the cost; the point cloud item type now owns its pipelines and builds them on the first frame that draws a cloud. Draw one offscreen frame with a cloud in it after loading if you were relying on the old timing.
...
### Fixes
- **`PointCloudRefItem::settings.pick_id` now picks** - a reference item's own pick id was ignored and the id baked into the upload was used instead, so two references to one stored cloud were indistinguishable and a reference to a cloud uploaded without an id could not be picked at all. The reference's id is now what the pick reports; set it on the reference, not on the item you upload.
