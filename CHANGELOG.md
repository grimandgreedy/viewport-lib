...
- **`ViewportGesture`, `BindingPreset` and `NavigationActions` are `#[non_exhaustive]`** - all three gained members for touch, and none of them could grow without breaking before. A `match` on the first two needs a `_ =>` arm, and `NavigationActions` is built through `Default` plus field assignment rather than a struct literal.
- **The default control scheme takes only the middle button and the wheel** - middle drag orbits, shift+middle drag pans, scroll zooms, ctrl+scroll orbits, shift+scroll pans. Left drag and right drag are no longer bound to the camera and belong to the application: selection, gizmo dragging, tools, context menus. Previously left drag orbited (in `ViewportPrimitives`) and right drag panned (in both presets). Two complete schemes, one for the mouse and one for a trackpad, with shift meaning pan and ctrl meaning orbit in both.
- **`BindingPreset::ViewportPrimitives` and `ViewportAll` are `Viewer` and `Default`** - the old names described breadth when the real difference was the mouse scheme. `Default` is the scheme above plus the viewport keyboard shortcuts; `Viewer` adds `Left drag -> Orbit` and is only for an application that never uses left drag itself. `viewport_primitives_bindings` and `viewport_all_bindings` are deprecated in favour of `viewer_bindings` and `viewport_default_bindings`, with `viewport_camera_bindings` for the camera half alone.
- **Undo and redo are no longer bound by the viewport** - they belong to the application's document, and binding them by default made every consumer notice and unbind them. Bind them yourself, or keep resolving them by adding the bindings back.
- **`OrbitCameraController::push_event` and `apply_to_camera` are deprecated** - a controller holding its own resolver is how a session and a controller end up disagreeing about the control scheme, which they silently did: a session resolves with its own bindings and ignores the controller's preset entirely. Own a `ViewportInput` (or a `ViewportInstance`) and drive the camera with `apply(&mut camera, &frame)`. `OrbitCameraController::new_stateless` is the constructor for that path. The preset constructors are deprecated with it: `viewport_primitives()` is renamed `viewer()`, and `viewport_all()` becomes `new_stateless()` on the session path or `new(BindingPreset::Default)` if you drive the controller's own resolver.
- **Overlay items carry one `clip` field** - `clip_rect` and `clip_id` on all four item types and on `RetainedOverlay` are replaced by `clip: OverlayClip { rect, mask }`; the `with_clip_rect` and `with_clip` builders keep their names and write into it. `OverlayShapeItem::clip_mask_id` becomes `provides_mask`, with its builder, because that field says other items are clipped by this one, which is the opposite direction of the relation. A compiled item's `clip.rect` is now baked at compile instead of being silently discarded, and an item naming a `clip.mask` at compile is skipped rather than drawn unclipped: masks are registered per frame and a compiled group has no frame to resolve one against, so clip the retained group instead.
- **Overlay anchoring is one field with two named halves** - `anchor`, `align_x` and `align_y` become `anchoring: OverlayAnchoring { origin, align }` on all four item types, and `Option<OverlayAnchoring>` on `RetainedOverlay`. `OverlayAnchor` is renamed `OverlayOrigin` and its `Viewport` variant takes an `Alignment { x, y }` rather than two fields, so the point on the viewport and the point on the item's own box are no longer spelled the same way on the same struct. `with_align` takes an `Alignment` on every type, `with_screen_anchor` exists on all four, and `LabelItem` keeps `with_align_x` / `with_align_y`.
- **An item's `tint` and `opacity` live in its style** - both move off the four item types into `OverlayStyle`, which is now the whole answer to how an item looks. `RetainedOverlay` keeps its own pair: those are live multipliers over a compiled buffer, so a fade stays a per-frame value rather than a re-compile. A retained group's `opacity` now reaches its shadow layers, so fading a compiled panel takes its drop shadow and its outline band with it; its `tint` reaches the fill and not the shadows, on both vertex streams.
- **A texture is a fill, and a fill is not optional** - `OverlayStyle::texture` and `texture_transform` and `OverlayShapeItem::nine_slice` are gone, replaced by `OverlayFill::Texture { id, transform, nine_slice, tint }`. `style.fill` is an `OverlayFill` rather than an `Option`, defaulting to a fully transparent solid, and `resolved_fill` goes with the `None` case. `with_texture(id)` stays as sugar; `with_texture_transform`, `with_nine_slice` and `with_texture_flip` go, because the variant holds those fields and a consumer edits them on it. `OverlayStyleSupport::fill` becomes the list of `OverlayFillKind`s a family draws, with `draws_fill` to ask, and `for_polyline` takes the item's `closed` flag: an open polyline has no interior, so a texture fill on one now reports as inert instead of being silently dropped.
- **A text item's colour is its fill** - `LabelItem::colour` and `GlyphRunItem::colour` are deleted. The two were multiplied with `style.fill` and the multiply was documented nowhere; `colour` defaulted to white, so the result was the fill. `with_colour` survives on both as sugar for a solid fill, and `with_fill` is new on both, which is what a gradient across text needs.
- **`GlyphRunItem::colours` is `glyph_tints`** - it stops being a colour once `colour` is gone: it is a per-glyph multiplier over the fill, `Vec<[f32; 4]>`, that behaves like `style.tint` and does not reach the shadow layers. `with_colours` becomes `with_glyph_tints`.
- **`LabelItem` and `GlyphRunItem` share a `text_style`** - `font` and `font_size` become `text_style: TextStyle { font, size }`. `with_font` and `with_font_size` keep working, and `with_text_style` sets both.
- **The polyline's stroke is one field** - `thickness`, `colour`, `join`, `mitre_limit`, `cap` and `stroke_pattern` become `stroke: Option<OverlayStroke>`. `None` draws no line, which is how a closed polyline with a fill and no outline is now spelled; it was `thickness: 0.0`. The per-field builders still work, and `with_stroke` / `without_stroke` set the bucket.
- **The label plate, the leader line and the anchor gap are deleted** - `background`, `background_colour`, `padding`, `border_radius`, `leader_line`, `leader_colour` and `anchor_padding` go, with their builders. The plate was a CPU-tessellated flat quad inside the text stream with no SDF, gradient, texture or shadow; a backing panel is an `OverlayShapeItem` with the same anchoring and a size of the measured text plus padding, which draws under the text at the same `z_order` by family rank. Labels shift by the deleted `anchor_padding`, which defaulted to `6.0` and applied whether or not a leader was drawn.
- **`with_border` is `with_outline` on every family** - the shape's border and the other three families' outline both push shadow layers, so they converge on `with_outline(colour, width, mode)`, and `BorderMode` becomes `OutlineMode` with `Center` spelled `Centre`.
- **Missing overlay builders filled in** - `with_style` on the label, glyph run and polyline; `with_inner_shadows` and `with_inner_shadow` on the same three, which have drawn inner shadows since the shadow-layer work but had no API for them; `with_shadow` and `with_inner_shadow` on the shape. `RetainedOverlay::with_translate` is renamed `with_position` and its `with_clip_mask` becomes `with_clip`, matching the item types.
- **Wrapped text can be measured** - `DeviceResources::measure_overlay_text_wrapped(text, font_size, font, max_width)` returns what a label with `max_width` set actually lays out to, from font metrics only. `LabelItem::resolve_top_left(size, ...)` and `GlyphRunItem::resolve_top_left(...)` resolve where an item lands for a frame, matching `OverlayShapeItem::resolve_top_left`, and `GlyphRunItem::extent` gives the box a run is aligned and turned against.
- **The overlay border fields are gone: a border is a shadow layer** - `OverlayShapeItem::border_colour`, `border_width` and `border_mode` are deleted, along with `with_border_mode`. `with_outline(colour, width, OutlineMode::Inset)` takes the mode up front and pushes the layers itself: `Inset` one inner layer, `Outer` one outer layer, `Centre` one of each at half the width. A band around a shape edge and a `ShadowLayer` with no blur and a spread were the same thing drawn by two code paths, and the measured difference between them was a one-pixel anti-aliasing seam that this removes by construction. Two consequences: a border costs a slot out of `OVERLAY_MAX_SHADOW_LAYERS` (four per list), and a vector path's border is now placed by `border_mode` rather than always straddling the contour, so an `Inset` border on one sits inside the edge as it already did on an analytic shape.
- **`OverlayStyleSupport` drops its `shadows` and `inner_shadows` fields** - both are true on every family now, so there is nothing left to branch on. A shadow layer means one thing everywhere: an outer layer dilates what the item covers and is clipped to outside it, an inner layer erodes it inward from the boundary. The remaining fields are `fill` and `backdrop`. `inert_fields` no longer returns either name. The falloff curves still differ between the three coverage backends, which the type documents and does not report.
- **More overlay types are `#[non_exhaustive]`** - `AnimTrack`, `TextureTransform`, `NineSlice`, `GradientStop`, `PositionedGlyph`, `SubPath`, `OverlayStyleSupport`, `RepeatMode` and `OverlayOrigin`. Struct literals on those are no longer available outside the crate; build them with the constructors instead, which is why this lands in the release that already breaks rather than costing a second one later. New for it: `AnimTrack::new` plus `with_easing` / `with_repeat` / `with_start_time` (it previously had no constructor at all), `TextureTransform::with_offset` / `with_scale` / `with_rotation` / `with_tile_mode` / `with_flip`, `NineSlice::new` / `with_centre_mode` / `with_edge_mode`, and `SubPath::from_segments` for converting a path in from another representation. `LineJoin`, `StrokePattern` and `PathSegment` are deliberately left exhaustive: the renderer dispatches geometry on all three, so a new variant should fail our build rather than fall into a `_` arm.
- **Overlay animation tracks are the five channels that survive compilation** - `opacity`, `translate`, `rotation`, `scale` and `tint`. The `size`, `fill` and `border` tracks are gone: driving one means re-emitting geometry, which defeats retention, and in immediate mode the consumer is rebuilding the item every frame and can set the field directly. A size pulse becomes a `scale` track and a colour cycle becomes a `tint` track, both of which ride the per-draw instance. `OverlayAnimations::position` is renamed `translate` to match `OverlayTransform`.
- **`PathTrack` is deprecated** - it holds an `Arc<dyn Fn>`, which makes it the one overlay type that cannot be serialised, compiled into a retained group, or sent across a plugin boundary. `OverlayEasing::CubicBezier` covers the curve cases; a genuine motion path is a `translate` you sample and set yourself. Only `translate_path` remains, and it goes away next release.
- **`OverlayEasing` is `#[non_exhaustive]` and no longer `Eq`/`Hash`** - it gained a variant carrying `f32`s. A `match` on it needs a `_ =>` arm.
- **`RetainedOverlay` is no longer `Copy`** - it carries an optional boxed animation block now. It is still `Clone`.
- **Overlay items share an `OverlayStyle`** - `fill`, `shadows`, `inner_shadows`, the texture fill and the four `backdrop_*` scalars moved off the item types into a nested `style: OverlayStyle`, with the backdrop scalars collected into a `BackdropEffects`. All four item types carry it, so a label or glyph run can hold a gradient fill and a polyline can hold an inset shadow; the builders (`with_fill`, `with_shadows`, `with_texture`, `with_backdrop_blur`, ...) keep their names and still work.
- **Overlay items and retained groups share one `OverlayTransform`** - `position`, `rotation` and `rotation_pivot` moved off `OverlayShapeItem`, `LabelItem`, `GlyphRunItem` and `OverlayPolylineItem` into a nested `transform: OverlayTransform { translate, rotation, pivot, scale }`, and `RetainedOverlay::translate` / `scale` moved into the same struct. The `with_position` / `with_rotation` / `with_rotation_pivot` / `with_translate` / `with_scale` builders keep their names and still work, so most call sites need no change; direct field access becomes `item.transform.translate` and so on. The composition rule between the two roles is written on `OverlayTransform`: a vertex resolves to `group_T . item_T . v`, two nested affine maps, which is not the same as adding the channels once a group rotates.
- **Every overlay item takes `scale`, a tint and a clip, honoured on both paths** - the three fields a retained group already had are now on the items too, so the same content looks the same whether it is drawn immediately or compiled. `scale` is uniform (see `OverlayTransform` for why), `tint` is a colour multiplier that never reaches shadow layers, and `clip_rect` is a framebuffer-space box that composes with `clip_id` by intersection.
- **A retained group can rotate** - `RetainedOverlay` gains `transform.rotation` and `transform.pivot`, applied on both retained streams (text and SDF shapes). The SDF stream also honours `transform.scale`, which it previously ignored: a compiled shape group submitted with a non-unit scale now changes size.
- **`clip.rect` is an `Option<[f32; 4]>`** - on `RetainedOverlay` and on the per-item clip. All-zero-means-no-clip is a GPU sentinel; leaking it into the public type made "clip to nothing" inexpressible and gave a zero-area clip at the origin the silent meaning "unclipped". `with_clip_rect` still takes a plain `[f32; 4]`.
- **The legacy single-shadow fields on `OverlayShapeItem` are gone** - `shadow_colour`, `shadow_radius`, `shadow_offset` and `shadow_inset`, along with `with_shadow` and `with_shadow_inset`. Use the stacked layer lists that superseded them: `with_shadows(vec![ShadowLayer::new(colour, blur, offset)])` for an outer shadow and `with_inner_shadows(...)` for an inset one.
- **`OverlayAnimation` and `OverlayShapeItem::animation` are gone** - the single-channel opacity enum is superseded by `animations`, which already took precedence over it. `FadeIn` becomes an `AnimTrack { from: 0.0, to: opacity }`, `FadeOut` the reverse, and `Pulse` the same track with `OverlayEasing::Pulse` and `RepeatMode::Loop`.
- **`OverlayShapeItem::animations` is an `Option<Box<OverlayAnimations>>`** - the track block is 400 bytes and almost no shape animates, so it is boxed out of the item: `OverlayShapeItem` drops from 736 bytes to 296. `with_animations` is unchanged and still takes an `OverlayAnimations` by value; only direct field access needs the `Some(Box::new(...))`.
- **`LabelItem::occlude` is gone** - it was documented as a no-op and rendered identically either way. Depth-aware overlay occlusion, if it arrives, is a feature for every world-anchored overlay family rather than a dormant flag on one type.
- **Built-in item types are named `vpl.*`, and that prefix is reserved** - they were `viewport.*`; the names are what `SceneFrame::submit_plugin_items`, `has_item_type_plugin` and `ViewportRenderer::plugin_resident_bytes` report and match on, so a string you hardcoded against one needs the new spelling. The prefix is now refused to anything else: the per-type calls on `ViewportRenderer` (`upload_sprite_set`, `create_gpu_particle_system`, and the rest) resolve their plugin by name and downcast it, so registering your own plugin under a built-in name replaced the built-in and turned those calls into panics. `with_item_type_plugin` refuses the collision where it is made, and the constant is `renderer::RESERVED_TYPE_NAME_PREFIX`. Only the exact prefix is refused, so `vpl_sprite` or `mycrate.vpl.sprite` are still yours.
- **The built-in colourmaps are resident from construction** - their LUT textures, views and `ColourmapId`s are created with the renderer instead of on the first `prepare`, so an upload that resolves a preset at startup now gets the real LUT rather than binding the neutral fallback for the life of the handle. Only the texels still wait for a queue, and they are written before anything draws. Two signatures follow: `builtin_colourmap_view` returns `&TextureView` instead of `Option<&TextureView>`, so drop the `unwrap_or`; and `builtin_colourmap_id_checked` is gone, because `builtin_colourmap_id` no longer has a state in which it can fail. `ensure_colourmaps_initialized` keeps its signature and is still worth calling if you read LUT texels before your first frame.
- **GPU particle and external instance set calls are renderer-only** - both item types now hold the content they draw, so `DeviceResources::create_gpu_particle_system`, `drop_gpu_particle_system`, `create_external_instance_set`, `drop_external_instance_set` and `set_external_instance_set_buffer` are gone. Call the identical method on `ViewportRenderer` instead: drop the `.resources_mut()` and nothing else changes. `DeviceResources` is a field of the renderer and cannot see the registered item types, so a forwarding shim was not an option.
- **External instances draw inside the opaque scene pass** - they had a render pass of their own that ran after the scene pass and after the hierarchical-Z depth capture, so their depth never reached the pyramid that feeds occlusion culling. They now draw with the other item types, which puts them in it: instanced meshes fully hidden behind external instances are culled on the following frame. They write depth, so this is the behaviour they should always have had, but a scene with both and occlusion culling enabled will cull more than it did.
- **Gaussian splat and marching-cubes uploads are renderer-only** - both item types now hold the content they draw, so `DeviceResources::upload_gaussian_splat`, `replace_gaussian_splat`, `free_gaussian_splat`, `begin_upload_gaussian_splat`, `upload_result_gaussian_splat`, `upload_volume_for_mc`, `free_mc_volume`, `set_mc_scalar_source_buffer`, `clear_mc_scalar_source`, `begin_upload_volume_for_mc` and `upload_result_volume_mc` are gone. Call the identical method on `ViewportRenderer` instead: drop the `.resources_mut()` and nothing else changes. `DeviceResources` is a field of the renderer and cannot see the registered item types, so a forwarding shim was not an option. `ViewportRenderer::replace_gaussian_splat` is new, completing that type's renderer-level surface.
- **The scivis upload calls are renderer-only** - polyline, point cloud, sprite, sprite instance set, glyph set, tensor glyph set, streamtube, tube and ribbon now hold the content they draw, so their `DeviceResources` upload, replace, drop and async methods are gone. Call the identical method on `ViewportRenderer` instead: drop the `.resources_mut()` and nothing else changes. Mesh, texture, volume and colourmap uploads are shared between item types and are unaffected. `PolylineGpuData`, `PointCloudGpuData`, `SpriteGpuData`, `GlyphGpuData`, `TensorGlyphGpuData` and `StreamtubeGpuData` are no longer exported: they were public but opaque, with no constructor, no methods and no public signature naming them, the same situation `VolumeGpuData` was in.
- **`ResidentBytes::scivis_bytes` reads zero** - those bytes are now reported through `plugin_bytes`, with the per-type breakdown in `ViewportRenderer::plugin_resident_bytes`. `total()` is unchanged. The field goes away in a later release, along with `gaussian_splat_bytes` and `mc_volume_bytes`.
- **`ResidentBytes::gaussian_splat_bytes` and `mc_volume_bytes` read zero** - those bytes are now reported through `plugin_bytes`, which sums what every registered item type holds. `total()` is unchanged, so a budget written against it needs no edit; a budget reading the two named fields should read `plugin_bytes`. Both fields go away in a later release.
- **Scatter animation takes its clock from you: `ScatterSettings::time_seconds`** - the renderer drove `NoiseDriver` scrolling and the refraction shimmer off an internal `Instant` started at construction, which was the only place in the library where wall-clock time reached a shader and changed pixels. Set `fd.effects.scatter.time_seconds` from your own elapsed seconds, the clock you already drive `GpuParticleSystemItem::time_step` and `ExposureSettings::dt` from. It defaults to `0.0`, so a volume with `noise` or `refraction` now holds still until you feed it: that is the visible change, and it only affects those volumes. In exchange a scatter frame is reproducible from its `FrameData`, an offscreen capture matches the frame it was captured from (the frame counter was already frozen for captures, but the clock was not), and both effects can be held to a golden image.
- **`MAX_SCATTER_VOLUMES` moved** - the per-frame scatter volume cap now lives with the scatter types, as `viewport_lib::MAX_SCATTER_VOLUMES` (or `scene::scatter_volume::MAX_SCATTER_VOLUMES`), instead of under `resources::volume::scatter_volume`. The value is unchanged at 16.
- **`RibbonItem` gains `depth_write`, defaulting to `true`** - a ribbon is a swept surface like the tube and streamtube beside it, and now draws with the opaque scene by default instead of always routing through order-independent transparency. That makes it visible to everything reading the depth buffer: it occludes, it receives projected decals, and soft-particle sprites fade against it. Set `depth_write: false` for a genuinely translucent ribbon to get the old OIT behaviour back; `Additive` ribbons never write depth either way.
- **`ScreenImageItem` is gone** - the screen-space image item type and `SceneFrame::screen_images` are removed, along with `march_implicit_surface`, `march_implicit_surface_colour` and `ImplicitRenderOptions`, which existed to feed it. A screen-space quad is an `OverlayShapeItem::textured_image` on a streaming `OverlayTextureId`; a quad fixed in the world is a textured mesh. The one thing with no direct replacement is per-pixel depth compositing of a CPU-rendered image against scene geometry: for implicit surfaces use `GpuImplicitItem`, which ray-marches on the GPU at full resolution and depth-tests against the scene.
- **`texture_slot_mismatches` returns an owned `Vec`** - the recorded list moved behind a lock so a texture binding built from a shared borrow can record a mismatch: item types prepare against `&DeviceResources` and could not reach it otherwise. Call `.as_slice()` on the result if you were passing it somewhere that wanted a slice; `texture_slot_mismatch()` and `clear_texture_slot_mismatches()` are unchanged.
...
- **Light colours are `Colour`** - `LightSource::colour`, `LightingSettings::sky_colour` / `ground_colour`, and the path tracer's `RtLight` variants were the last raw arrays in the public API. The magnitude was always a separate field, so the colour is pure chromaticity and a value off a picker or a Kelvin table is now expressible. `Colour::linear_rgb(r, g, b)` (or `.into()` on the array you have) keeps the current appearance exactly. Worth a look if you wrote `light.colour = [...].into()`: that resolved to the identity conversion before and does something now.
- **`replace_texture` takes a `TextureData`** - the in-place pixel swap recreated the texture as sRGB colour whatever it was, so replacing a normal map or a data map quietly reclassified it. It now takes the same payload as `upload_texture`, and the space and bind slot come from the data.
- **A texture bound into a slot that wants the other colour space is reported** - every texture slot documents the space it needs, the store records the space each texture was uploaded with, and a mismatch is logged at error level and readable from `texture_slot_mismatch()` instead of rendering a plausibly wrong image. `set_lightmap` returns it as an error outright. The fix is always the other constructor: `TextureData::normal_map` for a normal map, `TextureData::linear` for roughness, occlusion, or baked radiance.
...
- **`ItemTypePlugin` draw hooks drop their lifetime coupling** - `paint`, `paint_transparent`, `paint_depth_read`, `paint_foreground`, `outline_mask`, `cast_shadow_pass`, and `render_pick` now take `&self` and `RenderPass<'_>` instead of the `<'a>`-coupled forms; delete the lifetime from your impls. The coupling forced the pass to borrow the plugin for its whole recording and blocked dispatch into host-owned render passes.
- **`PickContext` and `RectPickContext` take a lifetime** - both now carry a `meshes` handle, so `ItemTypePlugin::pick` and `pick_rect` take `&PickContext<'_>` / `&RectPickContext<'_>`. Add the `<'_>` to your impls; nothing else changes.
- **`VolumeGpuData` is no longer exported** - it was public but opaque, with no constructor, no methods, and no public signature mentioning it, so nothing could be done with it. The per-frame data now lives with the volume item type.
...
### Changes
- **The camera controllers document what they read** - a table in `camera::controllers` naming which `ActionFrame` fields each one consumes, and the two facts consumers keep rediscovering: every controller here takes look from `navigation.orbit`, so it is a drag and not mouselook, and the character controllers ignore `pan`, `zoom` and `twist` entirely, so the wheel does nothing once one of them is active.
- **A conformance fixture covers the whole item-type seam at once** - `viewport-lib-testkit`'s `ConformanceItemTypePlugin` is an item type that stores its own content, uploads into it off the frame thread, draws in the scene, pick, shadow and outline-mask passes, answers a CPU pick, returns a bounds wireframe, and holds a bind group over one of your textures across frames, revalidating it with a `ResourceGate`. It is written against the public API and nothing else, so anything an item type needs that stops being publicly reachable fails to compile there. Read it when writing an item type of your own.
- **`ResourceGate` and `Revalidate` are public** - an item type that bakes a `TextureView` into a bind group pins that texture when the host frees it, and never sees a replacement swapped in behind a live id. Neither event is visible from the ids a plugin holds, so this is the part it cannot write for itself: keep a `ResourceGate` beside your store, `poll` it against `ctx.resources` at the top of `prepare`, and rebind the entries the `Revalidate` verdict names. Rebind rather than discard, and forget a dead id once you have. The built-in sprite, ribbon and particle types use exactly this.
- **`MeshDraw::draw_indexed_instance_range`** - draws one uploaded mesh over an instance range rather than a count, so `instance_index` in the vertex stage starts at the range's start. An item type whose instances are a window into a larger pooled buffer needs it; `draw_indexed_instanced` now calls it with `0..instances`.
- **Particle systems count toward `ResidentBytes::plugin_bytes`** - the particle buffers and their uniforms were resident GPU memory that no figure reported. They are usually the bulk of a particle-heavy scene's working set, so a budget written against `total()` will now see them. External instance sets deliberately charge nothing: the positions buffer belongs to the consumer, who allocated it.
- **`DeviceResources::check_texture_slot` is public** - it reports a texture bound into a slot that wants the other colour space, which an item type binding a consumer's texture should call and cannot implement itself (the mismatch log is the renderer's).
- **Stored scivis uploads build off the frame thread** - `begin_upload_point_cloud`, `begin_upload_sprite_set`, `begin_upload_glyph_set` and the rest previously did all their GPU work in the main-thread apply step; the buffer building now runs on the worker, with the colourmap views, samplers, textures and base meshes resolved on the calling thread and handed to it. `begin_upload_polyline` is the exception and still builds on the calling thread, because a polyline goes through the shared line substrate.
- **`ViewportRenderer::item_type_plugin_host`** - borrows a registered item-type plugin together with the job runner and read access to the shared content arenas, which is what an upload into a plugin-owned store needs. `item_type_plugin_mut` borrows the whole renderer, so `resources()` and the job runner are out of reach for as long as the plugin is held; these are separate fields and only the renderer can lend them out at once. The built-in types that hold their own content upload through this, so it is the same route an item type in your own crate has.
- **`ViewportRenderer::plugin_resident_bytes`** - the per-type breakdown behind `ResidentBytes::plugin_bytes`, in registration order. The sum is the figure to budget against; this is the one to act on, since an eviction policy over its ceiling needs to know which type to free from. Every registered type appears, including those reporting zero.
- **`ViewportInstance` reaches the item-type seam** - the runner forwards `with_item_type_plugin`, `item_type_plugin`, `item_type_plugin_mut`, `item_type_plugin_host`, `resident_bytes` and `plugin_resident_bytes`. Previously it could not register an item type at all without `renderer_mut()`. The per-type upload calls are deliberately not mirrored: `renderer_mut()` is the route for those.
- **`ViewportError::ItemTypePluginMissing`** - returned by an upload call whose owning item type is not registered with the renderer. The built-in types register at `ViewportRenderer::new`, so this only surfaces if you registered a plugin of your own under a built-in type name. `ViewportError` is matched exhaustively in some code; a new variant needs a `_ =>` arm.
- **`Jobs::try_submit_cpu`** - the plugin-facing job runner gained a fallible sibling to `submit_cpu`: the closure takes a `ProgressHandle` and returns a `Result`, and an `Err` surfaces as `UploadStatus::Failed` rather than a result that never arrives. This is what an item type with a validating upload needs to report failures the way the built-in uploads do.
- **Decals and scatter volumes answer GPU picks from their own item type** - both used to rasterise a shared proxy mesh from the renderer's pick pass; each now owns its proxy geometry and pick pipeline, and the last per-type branches are gone from the pick path. One behaviour change to know about: their pick bindings are built during `prepare`, so a GPU pick of a decal or scatter volume needs a prepared frame, the same as every other item type that answers the id pass with geometry of its own. Rendering before picking, which an application does anyway, is enough.
- **The plugin substrate documents who owns what** - `plugin_api`'s module docs now state the ownership line plainly: a plugin owns its buffers, layouts and pipelines, created from the `&Device` its hooks are given, and the library publishes no storage abstraction because a plugin needs none. What the library does supply is the part a plugin cannot build itself: off-thread work through `ItemFrameContext::jobs`, readers for content shared between item types, `resident_bytes` so an eviction budget can see what a plugin holds, and `item_type_plugin_mut` to reach a registered plugin again.
- **Uploads for the self-storing item types are on `ViewportRenderer`** - `upload_polyline`, `upload_point_cloud`, `upload_sprite_set`, `upload_volume_for_mc`, `create_gpu_particle_system` and the rest (with their `begin_upload_*` / `upload_result_*` / `replace_*` / `drop_*` companions) are now callable directly on the renderer, matching how Gaussian splats already worked. Drop the `.resources_mut()` from those calls when convenient: the `DeviceResources` methods still work and are unchanged. Mesh, texture, volume and colourmap uploads are shared across item types and stay where they are.
- **`ViewportRenderer::item_type_plugin` and `item_type_plugin_mut`** - borrow a registered item-type plugin back as its concrete type, by `type_name`. `with_item_type_plugin` takes the plugin by box and the renderer owns it from then on, so previously you could register a type and never speak to it again. This is the route for uploading into content an item type stores itself, changing its settings, or reading state it built during a frame. `ItemTypePlugin` gained a blanket-implemented `AsAnyItemTypePlugin` supertrait to make the downcast possible; existing implementations need no change.
- **`ItemTypePlugin::contribute_lights`** - a new optional hook returning lights an item type adds to the scene, appended after your own lights and subject to the same frustum cull and per-frame cap. An emissive scatter volume becomes a virtual point light through this hook now, rather than being special-cased in the renderer, so any item type can light the surfaces around it. Nothing changes for scatter volumes; the default returns no lights.
- **`PluginPipelineOpts::depth_bias`, and the shadow builder honours `depth_compare`** - `build_shadow_pipeline` pinned both, so a plugin casting shadows from thin or two-sided geometry got a bias suited to solid meshes and self-shadowed badly, with no way to change it short of hand-rolling the pipeline. `depth_bias` defaults to `None` (the previous fixed value), and `depth_compare` now applies to the shadow builder as well as the opaque one. The curve and isosurface item types build their shadow pipelines through the public builder as a result.
- **`DeviceResources::fallback_texture_view`** - the 1x1 neutral view the lib binds when a material slot names no texture, keyed by `TextureSlot`: white for albedo and AO, a flat tangent-space normal, `[0, 1, 1]` for metallic-roughness, black for emissive. A plugin building a bind group for an item with no texture in a slot can now bind exactly what the built-in types do.
- **`ItemTypePlugin::resident_bytes`** - a new optional hook for an item type that holds uploaded content of its own, summed into the new `ResidentBytes::plugin_bytes` by `ViewportRenderer::resident_bytes`. Accounting previously enumerated each store by name, so a plugin's own content was invisible to the working-set figure an eviction policy budgets against. Use `renderer.resident_bytes()` rather than `renderer.resources().resident_bytes()`: plugins are registered with the renderer, so the resources-level call reports `plugin_bytes` as zero.
- **Particle system and external instance set handles are generational** - `GpuParticleSystemId` and `ExternalInstanceSetId` were plain indices, but both stores reuse a dropped slot on the next create, so a handle held across a drop resolved to whatever landed in that slot next and silently drove the wrong system. Both now carry a generation and a stale handle resolves to nothing. Handles are opaque and still come from the create calls, so nothing changes at a call site.
- **`ItemTypePlugin::wireframe_polylines`** - a new optional hook returning the lines an item type draws under `settings.wireframe`, uploaded and drawn by the renderer through the shared line substrate, so a plugin needs no pipeline of its own for it. The four built-in types that draw synthesised lines (volume bounding box, a ring per Gaussian splat, a quad per sprite, scatter-volume bounds) now answer here instead of being special-cased in the renderer. This is also how an item type with no silhouette shows selection, which the outline ring cannot do for a volume or a cloud; `outline_mask`'s docs say when to reach for which.
- **Gaussian splat wireframe drops its large-set fallback** - a set of more than 100 splats drew a single box fitted around the whole cloud, which said less than the silhouette already did. Sets past that cap now draw no wireframe; at or under it, the ring-per-splat view is unchanged.
- **`obb_wireframe_polyline`** - builds a `PolylineItem` for the 12 edges of an arbitrarily oriented box from its 8 corners, alongside the existing `aabb_wireframe_polyline` and `sphere_wireframe_polyline`.
- **`ItemTypePlugin::sub_object_position`** - a new optional hook returning the world position of a resolved sub-object feature, which the renderer uses to snap a gizmo to it. Previously the renderer answered this itself for tubes, streamtubes, ribbons and point clouds by reading their frame items directly, so an external item type that resolved a `SubObjectRef::Point` got the ray hit position instead of the feature's own. Implement it if your type has point-like features; the default returns `None` and keeps the old fallback.
- **Scatter volumes are an item type plugin** - the participating-media item type moved onto the `ItemTypePlugin` seam with its four passes, pipelines and pick. Submission is unchanged: keep filling `SceneFrame::scatter_volumes`. The per-frame and temporal bind groups over the scene depth are now built per frame rather than cached against a size-derived key, which closes a hole where an HDR target reallocated at the same size left them pointing at a dead view. Emissive volumes still contribute virtual point lights and selected volumes still draw a bounds wireframe; neither moved.
- **Ribbon pipeline variants are selected correctly** - the variant set was built in one order and indexed in another, so a wireframe or additive ribbon could draw through a pipeline belonging to a different blend mode. Only reachable once a ribbon drew outside the OIT pass, which nothing did before this release.
- **A decal no longer disappears when the camera crosses its projection plane** - the planar decal shader discarded on `dot(view_dir, decal_Z)`, a camera-position test, so orbiting below the plane through the receiver removed the whole decal at once while its surface stayed visible. That test was a workaround from when the shader had no surface-orientation check; the surface check was reinstated later and covers the back-facing case on its own, because a visible surface always presents the side facing the camera. Only affects viewing angles that previously showed nothing: every decal golden but the one that orbits below is byte-identical.
- **Decals are an item type plugin** - the decal item type moved onto the `ItemTypePlugin` seam with its pipelines, pick, outline ring and stencil exclude pass. Submission is unchanged: keep filling `SceneFrame::decals`. Two side effects worth knowing: the unused LDR decal pipelines are gone (decals only ever composited on the HDR path), so renderer construction compiles three fewer pipelines; and the per-viewport decal depth bind group is now built per frame from the scene depth rather than cached on the viewport, which closes a hole where an HDR target reallocated at the same size left it pointing at a dead view.
...
- **`PointerOwnership` and `forward_to_viewport`** - say who won the pointer this frame (`Elsewhere`, `Inside`, `Owned`, `Grabbed`) and let the library decide what to forward. `ViewportEvent::is_contested` names the events that must go to exactly one claimant (a left press, its release, and the motion between); everything else is ambient and still reaches the camera, so the wheel keeps zooming while a tool drawn over the viewport holds the pointer. `ViewportContext::with_ownership` sets `hovered` and `focused` from one value so they cannot disagree.
- **`ViewportEvent::Touch`** - touch contacts reach the viewport as events, carrying a `TouchId`, a `TouchPhase` and a viewport-local position in logical pixels. `from_winit` maps `WindowEvent::Touch` and `from_egui` maps `Event::Touch`, both scaled and offset the same way as the pointer, so a contact is hit-testable against the same rects. Raw contacts rather than a host's derived pinch or pan: the viewport recognises the gestures itself so the feel is the same through every shell. A touch is contested like a left press, but an ending or cancelled contact is always forwarded, or a contact goes down and stays down when ownership changes mid-gesture. Both runners set `hovered` from a contact, since a touch device never sends `CursorEntered`.
- **Touch gestures resolve into the camera** - one finger orbits, two pan, a pinch zooms and a twist turns, recognised from the contacts by `ViewportInput` and resolved into the same `navigation` fields the mouse drives, so all six camera controllers get touch without changing. The gestures are bindable: `ViewportGesture::TouchDrag { contacts }` and `TouchPinch` are in both presets. `TouchSettings` holds what there is to calibrate - orbit, pan, pinch and twist sensitivity with per-axis inversion, the tap and long-press tolerances, the double-tap window - and is settable on `ViewportInput` and `ViewportInstance`. A third finger parks the gesture rather than distorting it, a contact lifting re-baselines instead of throwing the camera, and a host that reports both contacts and its own pinch or pan events has the latter ignored while fingers are down.
- **Taps, double taps and long presses reach `PointerFrame`** - `tapped`, `double_tapped`, `long_pressed`, `touch_position` and `contacts`, recognised by the library and interpreted by you, next to the existing `clicked`. Double tap and long press need a clock: call the new `begin_frame_at(ctx, time_seconds)` with your own elapsed seconds instead of `begin_frame`, as the `ViewportApp` runners now do. Without one they stay quiet and everything else still resolves.
- **`ViewportEvent::offset_by`** - the event with any viewport-local position shifted by `-origin`, for a viewport embedded in a larger window. Deltas are left alone. Worth using rather than matching the position-carrying variants yourself: that list just grew with touch, and a host that enumerated it by hand keeps compiling while leaving the new ones in window space.
- **`ViewportInput::cancel_gesture` and `is_gesture_active`** - end a gesture that will never see its release (the host taking the pointer, a mode change, a cancelled touch), and ask whether one is in flight before stealing it. Both are surfaced on `ViewportInstance`.
- **`ViewportInstance::with_bindings` and `set_bindings`** - choose the control scheme in the one place that resolves it.
- **Android and iOS example** - `crates/viewport-lib-examples/mobile` runs a viewport on a phone or tablet through winit and cargo-mobile2: one crate for both platforms, with touch mapped onto the built-in orbit controller. It sits outside the workspace because the generated Xcode and Gradle projects expect the crate's own target directory, so run it with `cargo mobile-example` on the desktop, or `cargo apple run` / `cargo android run` from that directory for a device. `scripts/check_mobile.sh` compile-checks both mobile targets without a device toolchain.
- **A retained overlay group can anchor** - `RetainedOverlay` gains `anchor`, `align_x` and `align_y`, resolved every frame like an item's: a viewport corner that follows a resize, or a world point projected through the camera that culls the group when it goes behind or off screen. Alignment shifts against the group's own compiled extent. A group compiled from a single `LabelItem` still carries that label's anchor; a submission that sets its own overrides it, so a compiled label can be re-anchored without re-compiling.
- **Gradient-filled text** - `style.fill` is honoured on `LabelItem` and `GlyphRunItem`, so a label or a glyph run can carry any `OverlayFill`, gradients included. It was the largest gap a consumer could not close themselves: a glyph run is rasterised internally, so there was no way to get a gradient onto text at any cost. The fill is evaluated per glyph vertex over the laid-out text box, which is exact for a linear gradient and piecewise-linear per glyph for a radial or conical one. A per-glyph `colours` entry multiplies into it, the same way `tint` composes. A textured glyph fill is still unsupported and `OverlayStyleSupport::for_glyphs()` reports it as such.
- **`OverlayFill::sample_in_box`** - evaluate a fill on the CPU at a point in an item's box, mirroring the gradient maths the SDF shader runs per fragment. This is what the glyph path uses, and it is public so a consumer can match a gradient in their own drawing.
- **`OverlayPolylineItem` rotates** - it was the last overlay family without rotation, on the argument that a polyline's geometry is the caller's point list so rotating it is their own affine transform. That holds immediately and fails under retention: the points are baked into the compiled buffer, so turning them means re-compiling. `with_rotation` and `with_rotation_pivot` now exist on it like every other family.
- **Easing is data: `OverlayEasing::CubicBezier { x1, y1, x2, y2 }`** - CSS `cubic-bezier()` semantics, evaluated by solving for the curve parameter at the given x. `Back` ships as a preset over it. `Bounce` and `Elastic` are separate native variants because a CSS cubic-bezier is one segment monotone in x and cannot express multi-oscillation curves, which is the same reason CSS added `linear()`.
- **Every overlay item and every retained group takes animation tracks** - `animations` was on `OverlayShapeItem` only; it is now on `LabelItem`, `GlyphRunItem`, `OverlayPolylineItem` and `RetainedOverlay` too, boxed so a static item pays a pointer. An animated retained group re-draws from its compiled buffers with no re-tessellation, because every channel a track drives rides the per-draw instance.
- **`OverlayAnimations::epoch`** - the time a track's `start_time` is measured from. `start_time` was an absolute time on an application-defined clock, which authored content cannot write down: a screen is authored long before the process runs. Set `epoch` when the animation begins and `start_time` becomes a delay. `AnimTrack::sample_from(time, epoch)` is the matching sampler, and `OverlayAnimations::apply` resolves a whole track set onto the transform, opacity, and tint it drives.
- **`OverlayStyleSupport` reports what each overlay family draws** - a small bitset answering "does setting this style field do anything here", queried per variant where a type has variants (`OverlayStyleSupport::for_shape` takes the `OverlayShape`, because an analytic shape has a distance field and `OverlayShape::Vector` does not). A lowering layer branches on it and emits a fallback instead of setting a field that silently does nothing. `inert_fields` lists the mismatches for a given style, and debug builds log each one once. The fields are the contract, not the curve: the three coverage backends compute a `ShadowLayer`'s falloff differently and are close rather than identical.
- **`OverlayPolylineItem::with_clip`** - a polyline can now name a clip mask the way overlay shapes, labels and glyph runs do, and its stroke and interior fill are clipped to that mask. Without it a path drawn inside a scroll container or a masked panel was the one overlay item that ignored the mask and drew across the whole surface; a caller working around that by trimming the path itself can stop. Textured interior fills are clipped too.
- **`EncoderScope::OnOpaqueSurfaces`** - a third encoder scope, between the opaque scene and the selection sub-highlight. The two positions are distinct because `hdr_sub_highlight` and the depth-read pass run between them, and both sides are load-bearing: stamping a projection effect at `AfterOpaque` paints over soft particles and sub-object highlights that overlap the receiver (measured at 7.8% of the frame, max channel delta 105), while moving `AfterOpaque` itself earlier stops refraction sampling those same soft particles (9.9% of the frame, max channel delta 201). Use `OnOpaqueSurfaces` for an effect that composites onto surfaces and `AfterOpaque` for one that samples the finished image. `EncoderScopeContext` gains `scene_stencil_only` (the stencil aspect, for masking against surfaces the effect must not touch), `outline_colour` / `outline_width_px` (for an item type drawing its own selection affordance), and `meshes` (to rasterise scene geometry into a mask).
- **`ItemTypePlugin::encode`, the encoder-scope hook** - an item type whose work is several passes over intermediate targets of its own, rather than draws inside a pass the lib begins, lists the points it wants through `encoder_scopes` and gets the frame's command encoder at each. `EncoderScopeContext` carries the scene colour (as a view to attach and a texture to copy from), the scene depth and its sampleable depth-only view, the shared camera bind group, the frame's `EffectsFrame`, and the scene size. Two scopes: `AfterOpaque` and `AfterTransparent`. HDR-only, like the OIT, depth-read and foreground hooks.
- **`shared_wgsl::SHARED_SHADOW_BINDINGS_WGSL`** - the group-0 declarations for the shadow-cast pass, which binds a different layout from every other pass. Prepend it instead of `SHARED_BINDINGS_WGSL` when writing a shader for `cast_shadow_pass`; without it there was no way to write one from outside the crate.
- **The streamtube, tube and ribbon item types are plugins** - the last of the curve types moved onto the `ItemTypePlugin` seam, each with its own pipelines, pick, outline mask, and (for ribbons) OIT and shadow casting. Submission is unchanged: keep filling `SceneFrame::streamtube_items` / `tube_items` / `ribbon_items` and their `*_refs` counterparts.
- **The curve pipelines are built on first draw, not on first submission** - `upload_streamtube`, `upload_tube` and `upload_ribbon` used to compile the render, OIT and shadow pipelines so a load-time upload absorbed the cost; the item types now own them and build them on the first frame that draws one. Draw one offscreen frame with the item in it after loading if you were relying on the old timing.
- **`ensure_glyph_base_mesh` builds from a shared borrow** - it took `&mut DeviceResources` and returned nothing, so a plugin could not reach the shared arrow / sphere / cube geometry from `prepare`, which holds `&DeviceResources`. It now takes `&self` and returns the `GlyphBaseMeshRef` directly, so the call site is one line instead of an install-time pre-build plus a per-frame `glyph_base_mesh` lookup. `glyph_base_mesh` still answers `None` before the mesh exists.
- **The scivis pipelines are built on first draw, not on upload** - `upload_point_cloud`, `upload_glyph_set`, `upload_tensor_glyph_set` and `upload_polyline` used to compile their render pipelines so a load-time upload absorbed the cost; the item types now own their pipelines and build them on the first frame that draws one. Draw one offscreen frame with the item in it after loading if you were relying on the old timing.
- **`pick_helpers::world_radius_in_pixels`** - the pixel radius of a world-space radius measured at a given world point, with the same 4-pixel floor and behind-the-eye fallback the built-in instance pickers use. For an item type drawn at a world size but picked by screen-space proximity.
- **The marching-cubes pipelines are built on first draw, not on upload** - `upload_volume_for_mc` used to compile them so a load-time upload absorbed the cost; the item type now owns them and builds them on the first frame that draws an isosurface. Draw one offscreen frame with an item in it after loading if you were relying on the old timing.
- **A hidden `GpuMarchingCubesItem` no longer renders** - the marching-cubes draw loop never checked `ItemSettings::hidden`, so setting it had no effect; it now skips the item like every other type.
- **The volume ray-march pipeline is built on first draw, not on upload** - `upload_volume` used to compile it so a load-time upload absorbed the cost; the volume item type now owns its pipelines and builds them on the first frame that draws a volume. If you were relying on the upload to absorb that compile, draw one offscreen frame with a volume in it after loading.
- **`MeshDraw` and `MeshGeometry` for item-type plugins** - an item type whose geometry is a consumer-uploaded `MeshId` can now use it: `PaintContext`, `OutlineMaskContext` and `PickPassContext` carry `meshes.draw_indexed(pass, mesh_id)`, which binds the vertex and index buffers and issues the draw (returning `false` on a stale id), and `PickContext` / `RectPickContext` carry `meshes.positions(mesh_id)` / `.indices(mesh_id)` for the CPU pick hooks.
- **`ItemFrameContext` carries the clip objects and the quality signal** - `clip_objects` for a plugin that bakes clipping into a uniform of its own rather than reading the group-0 bindings (a ray-marcher generally must, since it tests along the ray), and `quality_reduced` for one with a cost knob to turn down when the frame budget asks.
- **Plugins can answer the curve sub-object levels** - `PickMask::POLY_NODE`, `SEGMENT` and `STRIP` joined the shader-written levels, so a plugin whose pick fragment writes a node or segment index can refine to one in `resolve_sub_object`, and the pick pass now runs for plugins under a curve-only mask.
- **Plugins can answer the `VOXEL` sub-object level** - `PickMask::VOXEL` joined the shader-written levels alongside `INSTANCE`, `SPLAT` and `CLOUD_POINT`, so a plugin whose pick fragment writes an element index can refine to a voxel in `resolve_sub_object` on any device, and the pick pass now runs for plugins under a `VOXEL`-only mask.
- **`volume_view`, `volume_dims`, `builtin_colourmap_view` and `mesh_index_count`** - new `DeviceResources` accessors: the 3D texture view and grid dimensions of a field uploaded with `upload_volume`, the LUT view for a built-in colourmap preset, and the index count of an uploaded mesh (for dropping an item whose mesh is not resident during `prepare`).
...
- **`ItemTypePlugin::pick_rect`** - CPU box select now reaches plugin items: return a `PickRectResult` (object ids plus per-element `(id, SubObjectRef)` pairs) for items inside the rectangle, from state cached in `prepare`.
- **Item-type plugins prepare on the split viewport API** - `prepare_viewport` now dispatches plugin prepare and cull once per viewport, so plugin items render under `prepare_scene` / `prepare_viewport` / `paint_viewport` instead of silently vanishing there.
- **`plugin_api::pick_helpers`** - the projection and proximity routines the built-in CPU pickers use (`project_to_screen`, `pick_closest_polyline_segment`, `segment_in_rect`, `ray_triangle`, `ray_unit_box_toi`) are exported, so plugin picking matches the built-in tolerances instead of re-deriving them.
- **Instance-level GPU picking for plugins without `PRIMITIVE_INDEX`** - write the instance index from the vertex stage with `viewport_pick_instance_fs` (`SHARED_PICK_INSTANCE_WGSL`) and answer `INSTANCE` in `resolve_sub_object` on any device; when the primitive-index feature is missing the renderer now strips only the mesh-level bits from the mask instead of skipping plugin refinement entirely.
- **`ItemFrameContext` carries the frame flags and a resources view** - `wireframe_mode`, `outline_selected`, `sub_selection`, and read-only `DeviceResources` access during `prepare` / `cull`; `colourmap_view`, `fallback_colourmap_view`, `has_texture`, `glyph_base_mesh`, and `ensure_glyph_base_mesh` are new `DeviceResources` accessors to go with it.
...
- **Overlay text rotates** - `rotation` and `rotation_pivot` on `LabelItem` and `GlyphRunItem`, with the same contract `OverlayShapeItem` already has: radians about the text-box centre, pivot measured from that centre. A turned label turns its background plate and its shadows too, while its leader line keeps pointing at the world anchor.
...
- **`DecalItem::ambient`** - the constant ambient coefficient a lit decal adds to the hemisphere fill, the same role `Material::ambient` plays for a mesh. Defaults to 0.15 to match `Material`, so a decal on a default-material surface picks up the same floor; set it to the receiver's `ambient` where that has been changed.
...
- **Ask which material-texture path you got** - `ViewportRenderer::material_texture_binding()` returns `"bindless"` or `"per-batch"`. The choice comes from the device's enabled features, not its backend, so it is not inferable from the platform; the two paths bind textures differently and are worth naming in a bug report.

- **Mesh CPU geometry is optional and reported** - `set_retain_mesh_cpu_geometry(false)` stops uploads keeping a host-side copy of positions, normals, and indices, `release_mesh_cpu_geometry` / `release_all_mesh_cpu_geometry` drop what is already resident, and `ResidentBytes::cpu_geometry_bytes` reports the charge. The copies back CPU picking, clip-plane caps, and normal lines; turn them off if you use none of those. They are host memory, so `total()` still counts GPU bytes only and `combined()` is the figure to budget against on unified memory.
- **Ask which LOD group owns a mesh** - `lod_group_of_mesh(mesh_id)` returns the live group holding it as a level, so a consumer deciding what to evict can route a handle to `free_lod_group` rather than `free_mesh`.
- **Count how many nodes use a texture** - `Scene::texture_ref_count(texture_id)` is the texture counterpart of `mesh_ref_count`, checking all five material slots and submesh materials, so you can tell whether a texture is still needed before calling `free_texture`.

- **Set the pointer shape from the runner** - `FrameCtx::set_cursor` on both runners takes a `CursorShape` (grab, crosshair, resize, and the rest), so a host driving an interactive tool over the viewport can say what the pointer is over. Previously only `set_cursor_visible` was modelled, and only `ViewportAppV2` could reach past it through `raw_window`.

### Fixes

- **The eframe examples ask for a device they can use** - the examples crate turns on `showcase` by default, which enables `viewport-lib/raytrace`, which raises the renderer's storage-buffer floor from eight to ten. Eight is what eframe requests by default, so the nine examples that never set a `device_descriptor` aborted at startup on `ViewportRenderer::new`, `eframe-minimal` among them. They now go through one `viewport_lib_examples_eframe::native_options(size)` that asks for `recommended_device_limits` and `recommended_device_features`, which is what an eframe host embedding a viewport has to do anyway.

- **Trackpad pinch and pan now move the camera** - `from_winit` has always translated `PinchGesture` and `PanGesture` into `TrackpadPinch` and `TrackpadPan`, but the resolver discarded both, so a Mac trackpad rotated the camera and did not pinch-zoom. They resolve into `navigation.zoom` and `navigation.pan`, gated on hover like the wheel.
- **The egui adapter maps the trackpad gestures** - `egui::Event::Zoom` and `Event::Rotate` were both dropped, so an egui-hosted viewport had no twist while a winit-hosted one did. Both are mapped now, with the conventions normalised: egui reports zoom as a multiplicative factor where the viewport uses winit's additive log-scale delta, and egui's angle sign is the opposite of winit's, so the same physical gesture resolves the same way through either host.
- **Shaders that only Chrome rejects** - WGSL confines `textureSample`, `dpdx`, `dpdy` and `workgroupBarrier` to uniform control flow, and requires a return at the end of a value-returning function even after a `discard`. naga enforces none of that, Tint enforces all of it, so five modules built on every native backend and failed to compile in a browser, taking their pipelines with them: `overlay_shape_tex.wgsl` (textured overlay shapes), `sprite.wgsl` and `sprite_lit.wgsl` (soft particles, which took every sprite with them), `mesh_instanced.wgsl` and its OIT twin (instanced meshes, so most scenes drew nothing on the web), `volume_outline_mask.wgsl`, and `cull.wgsl`'s two compaction kernels. The textured fill's sample and the instanced emissive derivatives are hoisted above the branch that made them non-uniform, `compute_lit` takes its UV derivatives as parameters because it runs past a per-instance early return, the sprite depth fetch uses an explicit level, and the cull kernels guard their work instead of returning early ahead of a barrier. Rendering is unchanged everywhere else: the image goldens are exact.
- **`scripts/check_shaders.py` compiles the shader set in headless Chrome** - it hands every composed `.wgsl` to `createShaderModule` and fails on anything the browser rejects, which is the only way to see this class: a native build cannot. It skips the four pick shaders that resolve a hit to a face, edge or vertex, which use `@builtin(primitive_index)`, an extension Chrome does not expose yet, and says so rather than counting them as passes. GPU picking below object granularity therefore does not work on the web.
- **Nine storage buffers per stage no longer renders nothing** - `DEFORM_STORAGE_BUFFERS_PER_STAGE` gated per-vertex deformers at nine while the deform-enabled mesh pipeline layout binds ten in the vertex stage, so a device granted exactly nine turned deformers on and then failed the mesh, shadow, point-shadow, OIT and HDR pipeline layouts, leaving a viewport that drew nothing. The gate is ten, which is what it should have been since the uv1 stream took the eighth vertex-stage slot. Such a device now renders the base mesh path with deformers reported unavailable, as every other under-provisioned device already did. `ViewportGpuResources::new` derives the count from the layouts it builds and `debug_assert!`s it against the constant, so the next vertex-stage binding added fails a debug build instead of shipping. `ViewportInstance`'s storage-buffer warning also stops claiming construction will fail on devices where it will not: below the base path's floor it still does, and between that and the recommended headroom it now says which features are off instead.
- **Overlay text renders in Chrome** - `overlay_text.wgsl` called `textureSample` inside a branch on a per-vertex value, which WGSL only permits in uniform control flow because the call derives its own LOD. Tint rejected the module, so the text pipeline never built and no overlay text drew at all; naga does not enforce the rule, so the same build rendered text in Firefox. It now samples once with `textureSampleLevel` above the branch, which is exact rather than approximate: the glyph atlas has one mip level. `overlay_shape_tex.wgsl` has the same defect on its textured-fill path and is not fixed here, because an image fill can be mipped and wants the sample hoisted rather than pinned to level zero.
- **The web target builds and its CPU paths run** - `viewport-lib` did not compile for `wasm32-unknown-unknown` at all: every `ItemTypePlugin` impl failed the trait's `Send + Sync` bound, because the WebGPU backend's handles hold `RefCell`s and are `!Sync`. Every scientific item type was affected, which is to say the whole scivis surface. wgpu's `fragile-send-sync-non-atomic-wasm` feature is now on for that target, where the single-threaded guarantee it asks for holds. Three runtime traps went with it: `rayon` is no longer compiled into a wasm build at all (see `viewport_lib_types::par`), so extraction and iso-surface work runs serially instead of panicking on a thread pool that cannot be built; the upload runner's CPU stage runs inline there rather than on a worker, and `drain_until` no longer sleeps waiting for a thread that does not exist; and `capture_validation` no longer spins on a promise that only settles when the browser event loop turns, which would hang the tab. `scripts/check_web.sh` runs the whole thing in headless Chrome and fails if any step does.
- **Inset shadows draw on vector paths, polylines and text** - `inner_shadows` was honoured only by the analytic distance-field shader, so setting one on any other family did nothing. A tessellated item now takes an inset band running inward from its contour, and a label or glyph run takes an eroded copy of the letterform drawn over it. The one field an inset layer still ignores off the distance-field path is `offset` on an open stroke and on text, where there is no interior to slide the band within.
- **Textured overlay shapes take all four shadow layers** - the textured and backdrop-blur pipelines carried a single shadow on the vertex, so a textured shape got one layer where every other path got four, and it ignored that layer's `spread` and `falloff`. All three shape pipelines now read the same stacked layer buffer.
- **`BorderMode::Outer` drew nothing, and `Center` lost its outer half** - the SDF shape shader discarded every fragment outside the shape before the border band was evaluated, so only `Inset` (the default) ever worked. The band now keeps a fragment alive as far out as it reaches. A shape with an outer or centred border changes appearance: it gains the border it was asked for.
- **An outer shadow no longer shows through a translucent shape** - the layers are drawn behind the fill, and a dilated silhouette covered the whole interior rather than just the ring outside it, so a panel at `opacity` below 1 was tinted by its own drop shadow. The shadow is now clipped to outside the silhouette, matching CSS `box-shadow`, whose semantics `ShadowLayer::spread` already follows. This holds on all three coverage backends: the analytic distance field, the tessellated one (vector paths and polylines), which draws the layer as a band running outward from the contour rather than as a dilated silhouette behind the fill, and the glyph atlas, which bakes the cut into the cell.
- **Fill composited over a shadow used an approximation that only held for an opaque backdrop** - `mix(dst, src, src.a)` ignores the destination alpha, so a cleared or clipped shadow still leaked its colour into a translucent fill at full strength. It is a proper source-over now.
- **Sprites draw with the pipeline built for their own blend and lighting** - the keyed variant set was filled by iterating the key space and read back through a dense slot index, and the two enumerate in different orders, so eight of the twelve keys resolved to a pipeline built for a different key. An unlit `Additive` sprite resolved to an alpha-blended *lit* pipeline and failed device validation outright ("expects a BindGroup to be set at index 3"); the combinations that did not trip validation drew with the wrong blend mode, most visibly a lit `AlphaBlend` batch rendering additively. `Additive` and `Premultiplied` sprites, and every lit sprite, change appearance with this fix; `AlphaBlend` unlit, the default, was one of the four keys that happened to resolve correctly and is unaffected.
- **A texture freed or replaced under a pre-uploaded sprite batch or ribbon reaches the screen** - both bake a texture view into a bind group at upload and are drawn from it for as long as you hold the handle, so a `replace_texture` behind a live id never arrived and a `free_texture` left the batch pinning the texture and still sampling it. Each now rebinds against the current view when the resource epochs move: a replaced texture shows up, and a freed one leaves the batch drawn but untextured, which is what freeing it means. The sprites and the geometry stay: they are yours until you drop the handle.
- **A colourmap named by an upload that runs before the first frame is honoured** - the built-in LUTs were uploaded on the first `prepare`, but a point cloud, glyph set, tensor glyph set, streamtube, tube or ribbon uploaded at startup resolves its colourmap once and keeps what it read, so it bound the neutral fallback and stayed grey for the life of the handle. The upload calls now make the built-in set resident first.
- **GPU particle emission is reproducible** - the emit kernel had every dead slot race for the frame's spawn budget through an atomic, so which slots won was decided by GPU scheduling. Every attribute of a particle is seeded from its slot index, so the same scene produced different particles on every run: measured at up to 179/255 across about 9% of the frame. Slots are now chosen by a wrapping window from a per-system cursor, which is deterministic and drops the atomic. The emission rate is unchanged; a spawn whose window slot is still alive is skipped rather than moved to another slot, which only differs from before when capacity is below `rate * max_lifetime` and particles are being dropped regardless.
- **Refractive sprites render with supersampling on** - they used to be skipped outright whenever `ssaa_factor` was above 1, because the pass they drew in ran before the SSAA resolve and had no resolve of its own at the supersampled size. They now draw from the sprite item type's encoder hook, after the resolve, where the scene colour they sample is a finished image; the factor no longer affects them.
- **Soft particles no longer vanish under supersampling** - the soft fade turned the fragment position into a depth-texture coordinate by dividing by the scene viewport size, but the sprite passes draw into the supersampled attachments, so at `ssaa_factor = 2` every soft sprite sampled off the edge of the depth texture and faded to nothing. The divisor now comes from the bound texture's own dimensions, which is correct at any factor. The same correction went into the refractive sprite shader, where it is latent until that path runs under SSAA.
- **`paint_depth_read` output no longer disappears under supersampling** - the read-only-depth pass targeted the supersampled attachments, but it runs after the SSAA resolve, which is encoded once per frame; a plugin drawing there had its pixels written into a texture nothing reads again. The pass now targets the HDR views like the other passes on its side of the resolve. The hook was always dispatched, so the symptom was a draw that happened and then vanished.
- **Supersampling no longer drops decals, scatter and the other post-resolve passes** - the SSAA resolve carried colour only, so the HDR depth buffer was never written for the whole frame and every pass after the resolve failed its depth test against it. A decal that rendered normally vanished completely at `ssaa_factor = 2`. The resolve now downsamples the supersampled depth alongside the colour, taking the nearest sample of each block. Affects decals, the decal outline, the sub-object highlight, the read-only-depth pass, OIT, scatter volumes, the foreground pass, and `ItemTypePlugin::encode`.
- **`build_shadow_pipeline` builds against the shadow pass's own group 0** - it used the scene camera bind group layout, but the lib binds the cascade-space camera (a single dynamic-offset uniform) before calling `cast_shadow_pass`, so a plugin pipeline built with it could not draw into the shadow atlas. Plugins that cast shadows work now; nothing else changes.
- **Reference items pick under their own `settings.pick_id`** - `PointCloudRefItem`, `GlyphSetRefItem`, `TensorGlyphSetRefItem` and `PolylineRefItem` - a reference item's own pick id was ignored and the id baked into the upload was used instead, so two references to one stored cloud or set were indistinguishable and a reference to something uploaded without an id could not be picked at all. The reference's id is now what the pick reports; set it on the reference, not on the item you upload.
- **Item-type plugins draw in registration order** - they were held in a hash map and drawn in its iteration order, which is not stable from one process to the next, so two plugin types whose items overlap on screen could blend in a different order run to run. They now draw in the order they were registered.
- **A material holding a freed texture sampled another object's texture** - under the bindless material binding the array index was the texture's slot with the handle's generation discarded, so a material whose texture had been freed sampled whatever a later upload put in that slot. It showed up as one object wearing another's albedo and normal maps after a scene reload, on the devices that take the bindless path (Vulkan, DX12, and Apple silicon through Metal argument buffers). The index and the flag that says whether to sample it now come from one generation-checked lookup.
- **A freed texture handle now renders as an unset slot, not as the fallback texture** - `free_texture` said a material still holding the id would "fall back to the fallback texture". It now takes the scalar path instead, which is what the material actually asked for: a freed metallic-roughness map leaves `metallic` and `roughness` in charge, and a freed emissive map leaves the emissive colour in charge rather than forcing black. Albedo, normal and occlusion look the same either way, since their fallbacks were already neutral.
- **A decal holding a freed texture sampled the fallback as if it were real** - `has_normal` and the roughness, metallic and emissive flags came from whether the id was set rather than whether it still resolved, so a decal whose texture had been freed told the shader to sample a slot that was bound to the fallback. Emissive was the visible one: it read white instead of the decal's own colour. The flags now come from the same lookup that binds the view.
- **Freeing a texture reaches a cached decal** - the decal cache was keyed on the item alone, so a decal that had already been uploaded kept drawing a texture after `free_texture`, and kept it in VRAM through the cached bind group. Whether each texture still resolves is now part of the key, so the decal rebuilds against the fallback and the memory is released.
- **Six doc comments said bindless material textures need Vulkan or DX12** - they do not: the feature set is a property of the adapter, and Apple silicon reports it through Metal argument buffers, so a Mac takes the bindless path. Anyone deciding which path their device uses from those comments got the wrong answer.
- **`SkinningPlugin::attach_palette` is `#[must_use]`** - it returns `false` when the mesh has no attached weights, which skins the mesh against no palette: the geometry collapses and drags its UVs with it, so it reads as ruined textures rather than a broken pose. The skeleton feature now logs instead of dropping the result.
- **`TextureId::index()` documents that it is not a cache key** - a freed slot is reused by a later upload, so the index alone aliases two different textures; keying a cache on it, or truncating `raw()` to `u32`, samples another object's texture after the first free-and-reuse.
- **Decals are lit by the scene** - a `Replace` decal's texture is now treated as albedo and run through the same Cook-Torrance BRDF, cascaded shadows, and hemisphere ambient the opaque pass uses, instead of compositing raw albedo over an already-lit surface. Decals previously ignored the light entirely: they did not darken in shadow, did not track the key light, and were lit by light 0 alone in a multi-light scene, so a decal authored under one rig read wrong under any other. `Multiply` and `Additive` are unchanged and stay unlit, since neither carries an albedo. Expect decals to change appearance; if yours now read dark, they were relying on the unlit albedo and want a brighter texture rather than a lower alpha.
- **`free_mesh` refuses a LOD group member** - freeing a mesh that is a level of a live LOD group left the group pointing at a dead slot, which drew as a silently missing level rather than an error. It now frees nothing, returns `false`, and logs; free the group with `free_lod_group`, which drops each member no other group owns.
- **`set_pickable(id, false)` releases everything it should** - it dropped the CPU positions and indices but left the cached pick triangle mesh resident, so most of the memory stayed. It now clears the cache too, and the released bytes show up in `ResidentBytes`.
- **Frames are reproducible again with GPU-driven culling on** - the cull packed visible instances and draw lists in whichever order threads finished, so rendering the same scene twice gave images differing by 1 LSB on a few pixels. Both compactions now pack in instance order, for about 0.1 ms of GPU time per cull at 65536 instances on an RTX 3080. Reproducibility still needs your own submission order to be stable: fragment shaders take screen-space derivatives over a 2x2 quad, so draw order can move a low-roughness specular pixel by a few ULPs.
- **`read_debug_pixel` returns the surface you can see** - it read a per-fragment buffer that every fragment writes, so over overlapping geometry it reported whichever shaded last and could swap between unrelated surfaces while the image was stable. It now reads the HDR target after the depth test. It needs `PipelineMode::Hdr` and `DebugOutputMode::Replace` and returns `None` otherwise, and values are half-float rather than `f32`.
- **Debug visualisation no longer changes what it measures** - the per-fragment buffer was the reason the debug pipelines gave up early depth rejection, so a quantity read under debug vis came from a differently executing frame. Dropping that write restores early rejection, and the viewport-sized buffer it needed is gone along with its group-0 binding 12, which no longer appears in the shared bindings a plugin shader can name.
- **GPU-driven culling stays deterministic at any scene size** - the order-free compaction sized its chunk plan to a fixed 8192 batches and fell back to arrival-order submission past that, losing reproducible frames on scenes with many distinct meshes. The plan now sizes to the submission; only a scene needing more scratch than the device will bind as one storage buffer falls back.
