...
- Normal map decals (D2): set `DecalItem::normal_texture_id` to a tangent-space normal map and `normal_blend_strength` (0..1) to control how strongly it overrides the receiver surface normal. The receiver normal is approximated from screen-space depth derivatives; the decal normal map is decoded, rotated into world space using the decal's own tangent frame (derived from the model matrix inverse), and blended with the receiver normal. The shading difference is applied as a multiplicative factor on the decal colour so the result reads as an indent or emboss rather than a flat sticker.
...
- Roughness and metallic decals (D3): set `DecalItem::roughness` and `DecalItem::metallic` to add a specular approximation layer on top of the decal colour. Alternatively, supply `roughness_texture_id` or `metallic_texture_id` for spatially-varying PBR properties sampled from textures. A gloss highlight is computed using N.V as a retroreflection proxy: at low roughness a tight highlight appears at near-normal incidence; metallic tints the highlight by albedo. `DecalItem::sort_key` (i32) controls draw order within a frame: lower values draw first and are overwritten by higher values, enabling layered effects such as a wet-patch decal beneath gunshot craters.
- UV offset and scale (D4): `DecalItem::uv_offset` and `uv_scale` shift and scale the final texture coordinate before sampling, enabling sprite-sheet decals and scroll animations. Use `Scene::add_decal_animated` with `DecalAnimation::UvScroll { vx, vy }` to drive a continuously scrolling decal without per-frame updates. Decals with a finite lifetime are added via `Scene::add_decal_with_lifetime(item, lifetime_secs, fade_out_secs)`; the scene advances them with `update_decals(dt)` and removes expired entries automatically. Live decal items are collected each frame with `Scene::collect_decal_items()`.
- Receiver masking (D5): call `Scene::set_receives_decals(node_id, false)` to exclude a scene node from decal projection. Excluded surfaces write stencil 0 in a dedicated pre-decal pass; the decal shader discards fragments where stencil is 0. The stencil buffer is cleared to 1 at the start of the opaque pass so all surfaces receive decals by default. A slight negative depth bias (`constant: -2`) on the exclude pipeline ensures the LessEqual depth compare passes reliably when re-projecting the same geometry in a second render pass.
- Emissive decals: set `DecalItem::emissive` (f32, default 0.0) to add a self-illumination contribution on top of whatever the blend mode produces. The emissive colour is sampled from `DecalItem::emissive_texture_id` when set, or taken from the albedo colour otherwise. The contribution is always additive regardless of blend mode; `emissive = 0.0` has no effect. Values above 1.0 are meaningful in HDR and bloom under tone-mapping when `PostProcessSettings::enabled = true`.
- Soft projection-box edges: set `DecalItem::edge_fade` (f32, range 0.0-0.5, default 0.0) to fade the decal alpha to zero near the boundary of its projection box, hiding the hard rectangular cutoff at any viewing angle. The fade is computed per-axis using `smoothstep` in the fragment shader and multiplied into the final alpha. A value of 0.15-0.25 suits most decals; 0.0 preserves the previous hard-edge behaviour.
- Tri-planar projection: set `DecalItem::projection` to `DecalProjection::TriPlanar { blend_sharpness }` to sample the decal texture from all three local axes and blend by the surface normal, avoiding UV stretching on corners and non-planar surfaces. `blend_sharpness` (range 1.0-16.0; 4.0 is a good default) controls how quickly the blend transitions between face projections. Costs three texture samples per pixel instead of one; use planar projection (the default) on flat surfaces. Normal maps, roughness textures, and emissive textures fall back to the XY projection UV in tri-planar mode.
- Cylindrical projection: set `DecalItem::projection` to `DecalProjection::Cylindrical { facing }` to wrap a decal around a cylindrical surface. The decal's local Z axis is the cylinder axis; UV.x is the angle around the axis (0 at local +X, wrapping back at 360 degrees); UV.y is position along the axis (0 at the bottom, 1 at the top). The projection volume is a cylinder of radius 0.5 and height 1.0 in local space. `CylindricalFacing::Outward` (default) projects onto the outside of the cylinder (normals pointing away from the axis); `CylindricalFacing::Inward` projects onto the inside (normals pointing toward the axis). The facing check and radial rejection together prevent bleed onto flat surfaces inside the volume.
...
- Debug draw: plugins can submit transient and persistent debug visuals to a shared `DebugDraw` resource and render them as standard polyline, point cloud, and label items after each step. The API covers lines, points, AABB wireframes, sphere wireframes (three great circles), and world-anchored text labels. Each primitive carries a `DebugLayer` tag: `Dev` primitives are suppressed when `DebugDraw::dev_enabled` is `false`, making it straightforward to hide development-only visuals in ship builds without changing plugin code. Persistent primitives (keyed by a caller-assigned `u64`) survive `begin_frame()` until explicitly removed; transient primitives are cleared by the next `begin_frame()` call. After `step()`, call `to_polylines()`, `to_point_cloud()`, and `to_labels()` to produce renderer-ready items. Showcase 46 demonstrates the full path with a simulated physics scene.
- Async job handoff: plugins can now perform background work through `JobSlot<T>` and `JobSender<T>`. Call `JobSlot::new()` to get a linked (slot, sender) pair, move the sender to a background thread, then poll `slot.take()` in `collect` each frame until the result arrives. Results transition through `JobPoll::Pending`, then `Ready(T)`, `Failed(String)`, or `Cancelled`. Dropping the sender without signaling auto-cancels. A `JobSlot` stored in `RuntimeResources` can be shared across plugins.
...
- Skeletal animation substrate: CPU linear blend skinning is now available as a built-in runtime plugin. Define a `Skeleton` (joint hierarchy with bind-pose inverse matrices) and set a `Pose` in `RuntimeResources` each frame from an animation plugin. `SkeletonPlugin` runs at `POST_SIM`, reads the pose, runs forward kinematics, and pushes a `SkinnedMeshUpdate` to `output.skinned_mesh_updates`. After `step()`, iterate the updates and call `renderer.resources_mut().write_mesh_positions_normals(queue, id, &pos, &nrm)` to upload deformed geometry. No shader or pipeline changes are required. `SkinWeights` (per-vertex joint indices and blend weights) is stored in `MeshData::skin_weights`. Showcase 47 demonstrates a two-joint bending arm.
- `SkinWeights` invariants are now spelled out: per-vertex index and weight arrays must match the mesh's vertex count, unused influence slots carry weight `0.0` with any in-range index, weights are expected to sum to `1.0` (the CPU path does not renormalise, so under-weighted vertices deform with reduced magnitude), and the four influence slots are order-independent. Importers should normalise before constructing.
- The path to GPU skinning is documented on `SkinWeights`. Joint indices and weights will be promoted to first-class vertex attributes (`JOINTS_0: [u16; 4]`, `WEIGHTS_0: [f32; 4]`) in a future release; the builder shape of `SkinWeights` will be preserved.
- Animation clips: an `AnimationClip` is a list of `Track`s, where each track binds a `Sampler` (keyframe times paired with values plus an interpolation mode) to one channel (translation, rotation, or scale) on one joint. `AnimationClip::sample_into(t, &mut pose)` applies all tracks at time `t` to a pose, overwriting only the animated channels and preserving the rest from the input pose. `Interpolation::Linear` (vec3 lerp / quaternion slerp) and `Interpolation::Step` are supported; cubic-spline is reserved for a later release.
- `ClipPlayerPlugin`: feeds a `Pose` into `RuntimeResources` each frame by sampling an `AnimationClip` over a bind pose. Runs at `phase::ANIMATE` so `SkeletonPlugin` sees the pose at `POST_SIM`. Configurable speed, looping, and play/pause; the playhead is exposed for manual seeking. Showcase 47 includes a "Clip-driven arm" entry that uses a five-keyframe rotation clip on the forearm joint, with the bind-pose translation supplied separately so the clip only has to carry what changes.
- glTF skinned-mesh import via `viewport-lib-io`. The glTF loader in viewport-lib-io now extracts per-vertex joint indices and weights (`SurfaceMesh::skin_weights`), per-skin joint hierarchies with inverse bind matrices (`SceneData::skeletons`, joints emitted in topological order), and per-skeleton animation clips (`SceneData::animations`, one clip per skin per glTF animation). Linear and Step interpolation are consumed by downstream players; CubicSpline is preserved on import for round-trip fidelity but not yet sampled. Up to four influences per vertex; joint indices above 255 are clamped (Phase 5 of the skeletal plan will widen to u16). Morph-target weight channels are skipped. Showcase 47 adds a "glTF character" entry that loads a skinned `.glb` from `examples/eframe_showcase/assets/character.glb` if present, with a graceful placeholder + instructions when the file is absent.
- Bone-parented rig auto-skinning. When the glTF loader encounters a mesh node that has no skin reference and no JOINTS_0/WEIGHTS_0 attributes but lives under a joint in the scene graph, it now synthesises 100%-weight skinning to the nearest joint ancestor. Vertex positions and normals are pre-baked to bind-world space so the skinning output at bind pose equals the original world position. This makes the common "rigid bone-parented" character export (one mesh per body part glued to a bone) drive correctly through the same `SkeletonPlugin` + `ClipPlayerPlugin` path as a real skinned mesh.
- `SkinnedActorPlugin`: drives many independently-animated actors from a single shared skeleton. Each `SkinnedActor` has its own clip, playhead, speed, and play state; within an actor, all `SkinnedActorPart`s deform from that actor's pose so multi-mesh characters stay in sync internally. One plugin runs the whole crowd through forward kinematics + LBS in a single phase tick. Showcase 47 adds a "Crowd" entry that instances the glTF character on a grid with a slider for actor count (1..32), staggered playheads, and a clip choice cycled per actor. CPU cost grows linearly with `actors * total_vertices`; Phase 5 (GPU skinning) will replace the per-actor vertex-buffer writes with one joint-palette upload per actor.
- GPU skinning vertex stage: the renderer now ships a skinned variant of the standard lit pipeline and the shadow-cast pipeline. Linear blend skinning is computed in the vertex shader from a per-mesh weights sidecar (joint indices and weights packed at 24 bytes per vertex) and a per-instance joint palette of `mat4x4<f32>`. The bind-pose vertex buffer is never modified. Public API: `ViewportGpuResources::set_skin_weights(device, mesh_id, &SkinWeights)` attaches the sidecar; `set_skin_palette(device, queue, mesh_id, instance_id, &[Mat4])` uploads the joint palette for the next frame; `supports_gpu_skinning() -> bool` is the capability flag. `SceneRenderItem::skin_instance: Option<u32>` selects which uploaded palette an item draws from, so many actors can share one bind-pose mesh. Static meshes pay zero overhead; only meshes marked skinnable via `set_skin_weights` allocate the sidecar storage buffer.
- `SkinnedPoseUpdate` output channel parallel to `SkinnedMeshUpdate`. Skinning plugins now expose a `path: SkinningPath` field (`Cpu` or `Gpu`). On `Cpu` (default) the plugin runs LBS on the CPU and emits the existing `SkinnedMeshUpdate`; on `Gpu` it emits `SkinnedPoseUpdate { mesh_id, instance_id, joint_matrices }`, which the host drains into `set_skin_palette`. `SkinningPath` is a builder option on `SkeletonPlugin` (`.with_path(Gpu)`) and `SkinnedActorPlugin`. For `SkinnedActorPlugin`, each actor's index becomes its `instance_id` so a crowd of N actors maps to N joint palettes.
- `MeshData::skin_weights` is removed; `SkinWeights` is a standalone type. Consumers that previously populated `MeshData::skin_weights` now call `ViewportGpuResources::set_skin_weights(device, mesh_id, &weights)` after `upload_mesh_data`. CPU skinning plugins continue to hold their own copy of the weights, so nothing changes for callers that only use the CPU path.
- `Scene::set_skin_instance(node_id, Option<u32>)` propagates the per-node skinning instance to `SceneRenderItem::skin_instance`. Use `Some(instance_id)` to route a node through the skinned pipeline once a palette has been uploaded for that `(mesh_id, instance_id)`; use `None` to draw it through the static pipeline even if the mesh has skin data attached.
- Showcase 47 gains a CPU/GPU toggle in the sidebar. Switching forces every active demo onto the selected path and rebuilds the runtime so the plugins emit the right output channel. A smoothed `runtime.step()` microsecond readout sits next to the toggle so the crowd-demo difference is visible.
- Skinned transparent and wireframe pipeline variants land alongside the standard lit + shadow variants from 5.2. Skinned meshes drawn with `opacity < 1.0` or a blended material now go through `skinned_transparent_pipeline` (alpha-blend, no depth write, no cull) instead of being mis-rendered through the opaque variant. Wireframe-mode draws of skinned meshes go through `skinned_wireframe_pipeline` (LineList over the edge index buffer). Both have HDR counterparts (`hdr_skinned_transparent_pipeline`, `hdr_skinned_wireframe_pipeline`) created alongside the other HDR variants. The LDR per-object path, the LDR instanced excluded-items path, the LDR instanced wireframe-fallback path, the HDR per-object path, the HDR opaque-excluded-items path, and the HDR wireframe path all route correctly. PBR and matcap shading are handled automatically because they are per-material switches in the fragment stage that the skinned variant reuses unchanged.
- `mesh_skinned.wgsl` now consumes the per-vertex storage buffers that `mesh.wgsl` does: `warp_buffer` (vertex displacement), `scalar_buffer` (per-vertex scalar field), `face_colour_buffer` (per-face RGBA). Warp is applied to the bind-pose position before skinning, so a warped vertex follows its joint correctly. Skinned meshes now respect `warp_attribute`, `active_attribute`, and `use_face_colour` exactly like static meshes. Previously these features were silently defaulted to inert on the GPU path.
- Skinned items are excluded from instanced batches and drawn through the per-object skinned pipelines instead. The instanced shadow pass also picks up per-object skinned shadow draws after its batched loop so skinned meshes still cast shadows when instancing is on. True batched-skinning (a per-batch palette array indexed by `@builtin(instance_index)`) is a perf follow-up; the current routing is correct but not batch-optimised.
- `skinned_oit_pipeline` lands as a skinned variant of `oit_pipeline` (HDR OIT accumulate/reveal targets). The HDR OIT pass routes skinned transparent items through it, switching between static and skinned variants lazily within the pass.

### Added
- SDF overlay shapes: `OverlayShapeItem` renders screen-space shapes with signed-distance functions evaluated per fragment, producing anti-aliased edges and inset borders without CPU tessellation. Eight shape types are supported through the `OverlayShape` enum: `Rect` (uniform corner radius), `RoundedRect` (per-corner radii), `Circle`, `Ellipse`, `Capsule`, `Ring` (hollow circle with configurable wall thickness), `Arc` (annular sector with start/end angles for progress indicators and radial menus), and `Triangle` (cardinal-directed isoceles via `TriangleDirection`). Submit shapes via `OverlayFrame::shapes`; they are drawn before rects and labels in all render paths. The existing `OverlayRectItem` is unchanged.
- Texture-masked overlay shapes: `OverlayShapeItem::texture` accepts an `OverlayTextureId` returned by `ViewportGpuResources::upload_overlay_texture`. When set, the SDF boundary, border, and AA all apply as normal but the interior samples the uploaded image rather than a solid colour. `fill` acts as a per-channel tint multiplied with each texel; set it to `OverlayFill::Solid([1, 1, 1, 1])` for no colour shift. Textures are uploaded once and referenced by ID across frames. Shapes using different textures are batched per texture; solid shapes are unaffected and continue through the existing pipeline. Use cases include circular avatars, rounded-corner images, and textured HUD panels.
- Gradient fill for overlay shapes: `OverlayShapeItem::fill` is now an `OverlayFill` enum replacing the former `colour: [f32; 4]` field. Use `OverlayFill::Solid(colour)` for the previous solid fill behaviour, or `OverlayFill::LinearGradient { start_colour, end_colour, angle }` for a two-colour linear gradient. The gradient is computed in the fragment shader by projecting `local_pos` onto the gradient direction and mixing across the full bounding box extent. `angle = 0.0` produces left-to-right; positive angles rotate counter-clockwise. SDF boundary, border, and AA are unaffected by fill mode.
- Shadow/glow for overlay shapes: `OverlayShapeItem` gains `shadow_colour`, `shadow_radius`, and `shadow_offset` fields. When `shadow_radius > 0`, the fragment shader evaluates the SDF a second time at the offset position and uses `smoothstep` to fade from `shadow_colour` to transparent over `shadow_radius` pixels outside the shape boundary. The bounding quad expands automatically to accommodate the shadow. Works with all shape types, fill modes, and texture fills. Set `shadow_offset` to `[0, 0]` for a centred glow or to positive values for a directional drop shadow.
- Hit testing for overlay shapes: `OverlayShapeItem::contains(point)` and `OverlayShapeItem::distance(point)` evaluate the same SDF on the CPU to test whether a screen-space point (logical pixels from viewport top-left) falls inside a shape. `distance` returns the signed distance to the boundary (negative inside, positive outside). Works with all eight shape types including per-corner rounded rects, rings, arcs, and triangles.
- Border mode for overlay shapes: `OverlayShapeItem::border_mode` controls where the border sits relative to the shape edge. `BorderMode::Inset` (default, unchanged behaviour) draws the border inside the fill. `BorderMode::Outer` draws it outside, and `BorderMode::Center` straddles the edge. No vertex layout changes; the mode is encoded in `shadow_params.w`.
- Opacity animation for overlay shapes: `OverlayShapeItem::animation` accepts an `OverlayAnimation` enum with `FadeIn`, `FadeOut`, and `Pulse` variants. Animations are resolved during `prepare()` using `OverlayFrame::time` (seconds from an application-defined epoch). No shader changes; the animation modulates the final opacity before vertex construction. Note: the host application must request continuous repaints while animations are active since viewport-lib does not own the event loop or repaint schedule.
- Four new overlay shape types: `Line`, `Star`, `RegularPolygon`, and `Cross`. `Line` draws a stroked segment between the top-left and bottom-right corners of its bounding box with a configurable `thickness` and `LineCap` (round or square). `Star` renders an N-pointed star with independent outer and inner radii (`inner_radius_frac`). `RegularPolygon` draws a convex N-sided polygon (pentagon, hexagon, etc.) inscribed in the bounding box. `Cross` draws a plus shape as the union of a horizontal and vertical rectangle controlled by `arm_width_frac`. All four support the existing fill, gradient, border, shadow, animation, and hit-testing features. The `Line` bounding quad expands automatically by `thickness / 2` so the stroke is never clipped at the segment endpoints. CPU hit-test functions (`contains` / `distance`) cover all new shapes.
...
- HDR non-culling instanced pipeline is now created when geometry first appears, not only on the first frame. Previously, starting with an empty scene (e.g. a main menu) and spawning geometry later left `hdr_solid_instanced_pipeline` as None because the one-time init guard in `ensure_hdr_shared` had already fired before `instance_bind_group_layout` existed. Pipeline creation now lives in `instancing.rs` with its own lazy guard, matching the pattern the culling path already uses.
- `set_render_scale` now works on the HDR render path. Previously the doc comment stated "has no effect on the HDR render path" and while scene-side textures were allocated at scaled resolution, the tone-map and FXAA passes still ran at native resolution, producing no visible change. Tone-map and FXAA now run at scene resolution; an upscale-blit pass copies the result to native resolution before post-tone-map passes (grid, ground plane, gizmos, overlays).

### Improvements
- Render scale and dynamic resolution now work in both the LDR and HDR paths. All scene-side textures (HDR colour, depth, bloom, SSAO, DoF, contact shadows, FXAA, LIC, outlines) are allocated at `render_scale * surface_size`. Tone-map and FXAA run at the scaled resolution; an upscale-blit pass copies the post-processed result to native resolution before post-tone-map passes (grid, ground plane, gizmos, axes, overlays). A depth blit pass copies the scene-resolution depth buffer to a native-resolution copy so those passes draw at full resolution. `FrameStats::render_scale` now reflects the actual scale in both modes.

- Scene traversal acceleration: `Scene::collect_render_items_culled` now uses a loose octree spatial index when the scene has 500 or more nodes. Subtrees whose loose bounds are entirely outside the frustum are skipped without visiting any entries, reducing CPU traversal cost sub-linearly with node count. Scenes below the threshold continue to use the existing flat walk. The octree is built on first use and updated incrementally: adding, removing, or transforming nodes marks only the affected entries dirty; the dirty set is flushed at the start of each cull pass without rebuilding the full tree. Call `scene.rebuild_spatial_index(mesh_aabb_fn)` after a bulk scene load to do a single build rather than accumulating N incremental updates. `scene.scene_stats()` returns `SceneStats { spatial_index_nodes, frustum_cull_traversal_ms }` for diagnostic use.
...
### Fixes
- `ContactEvent` now includes a `contact_point: glam::Vec3` field giving the world-space position of the collision. Physics plugins can populate this for use in sound, particle effects, and decal placement. Simple plugins that do not compute a contact point can leave it as `Vec3::ZERO`. `PhysicsLitePlugin` now populates it with the body's position at the moment of contact.
- `SceneFrame::from_surface_items` now has a `with_generation(u64) -> Self` builder method. Without it, frames built from manually assembled render items (such as physics-interpolated transforms) would freeze after the first frame because the renderer's instance buffer cache saw an unchanging generation of `0`. Pass `scene.version()` when items are derived from runtime writeback. Static scenes that never change their items are unaffected.
- The `plane` primitive now lies in the XY plane with a +Z normal, consistent with the Z-up coordinate convention used throughout the library. Previously it lay in the XZ plane with a +Y normal.
- Ten additional primitives have been corrected to match the Z-up convention: `cylinder`, `cone`, `capsule`, `arrow`, and `spring` now extend along the Z axis (previously Y); `disk`, `ring`, `grid_plane`, `torus`, and `hemisphere` now lie in the XY plane with a +Z normal (previously XZ plane with a +Y normal). Triangle winding is preserved on all affected meshes.
