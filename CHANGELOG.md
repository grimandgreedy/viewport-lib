...
`ObjectUniform` grows by 16 bytes (one `u32` flag word plus tail padding) and the renderer carries a new `DeformationState` resource with a group(2) bind group layout, a shared header uniform, a renderer-owned dummy bind group for unattached meshes, and per-mesh storage buffers + bind groups that swap the dummy in for any unattached slot. Registration and pipeline rebuild are not yet wired through, so the identity path is still what runs at draw time; the registry validates and stores the descriptor, and the attach API uploads data the next pipeline rebuild will pick up.
