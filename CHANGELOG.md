...
- **The `DeviceResources` upload calls for the self-storing item types are deprecated** - `upload_polyline`, `upload_point_cloud`, `upload_sprite_set`, `create_gpu_particle_system` and the rest (50 methods across the nine scivis types, GPU particles and external instance sets) now warn and name their `ViewportRenderer` replacement. Dropping the `.resources_mut()` is the whole edit; signatures and behaviour are identical. They go away in a later release, and each one goes when the content it uploads moves to the item type that draws it, which `DeviceResources` cannot reach. Mesh, texture, volume and colourmap uploads are shared between item types and are not affected.
