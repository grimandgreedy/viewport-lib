...
- **Shared layer mask: visibility, per-camera cull, and light channels.** One
  ...
  which layers an item belongs to; `CameraFrame::cull_mask` (or
  `CameraFrame::with_cull_mask`) sets which layers a camera draws, culling any item
...
- **Second UV set (`TEXCOORD_1`).** `MeshData` gains
  ...
  Works on the per-object and instanced paths, including bindless textures and
  GPU-driven submission: the instanced colour bind group binds the chunk's uv1
  stream per vertex-slab chunk, so a second-UV-set material renders the same on
  every path.
