...
- Opacity animation for overlay shapes: `OverlayShapeItem::animation` accepts an `OverlayAnimation` enum with `FadeIn`, `FadeOut`, and `Pulse` variants. Animations are resolved during `prepare()` using `OverlayFrame::time` (seconds from an application-defined epoch). No shader changes; the animation modulates the final opacity before vertex construction.
