...
- Skeletal animation substrate: CPU linear blend skinning is now available as a built-in runtime plugin. Define a `Skeleton` (joint hierarchy with bind-pose inverse matrices) and set a `Pose` in `RuntimeResources` each frame from an animation plugin. `SkeletonPlugin` runs at `POST_SIM`, reads the pose, runs forward kinematics, and pushes a `SkinnedMeshUpdate` to `output.skinned_mesh_updates`. After `step()`, iterate the updates and call `renderer.resources_mut().write_mesh_positions_normals(queue, id, &pos, &nrm)` to upload deformed geometry. No shader or pipeline changes are required. `SkinWeights` (per-vertex joint indices and blend weights) is stored in `MeshData::skin_weights` for reference. Showcase 47 demonstrates a two-joint bending arm.
