...
- **Overlay items share an `OverlayStyle`** - `fill`, `shadows`, `inner_shadows`, `texture`, `texture_transform` and the four `backdrop_*` scalars moved off the item types into a nested `style: OverlayStyle`, with the backdrop scalars collected into a `BackdropEffects`. All four item types carry it, so a label or glyph run can hold a gradient fill and a polyline can hold an inset shadow; the builders (`with_fill`, `with_shadows`, `with_texture`, `with_backdrop_blur`, ...) keep their names and still work. `nine_slice` stays on `OverlayShapeItem`, because it is a rect-quad stretch scheme with no meaning on the other families.
- **`OverlayStyle::fill` is an `Option<OverlayFill>`** - `None` draws no fill, which is what a stroked-only polyline already meant and is now expressible on a shape too. `OverlayShapeItem::default` still fills solid white.
...
- **`clip_rect` is an `Option<[f32; 4]>`** - on `RetainedOverlay` and on the new per-item field. All-zero-means-no-clip is a GPU sentinel; leaking it into the public type made "clip to nothing" inexpressible and gave a zero-area clip at the origin the silent meaning "unclipped". `with_clip_rect` still takes a plain `[f32; 4]`.
