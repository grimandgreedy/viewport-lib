...
- **The point cloud, glyph and tensor glyph pipelines are built on first draw, not on upload** - `upload_point_cloud`, `upload_glyph_set` and `upload_tensor_glyph_set` used to compile the render pipelines so a load-time upload absorbed the cost; the three item types now own their pipelines and build them on the first frame that draws one. Draw one offscreen frame with the item in it after loading if you were relying on the old timing.
...
### Fixes
- **`PointCloudRefItem`, `TensorGlyphSetRefItem` and `GlyphSetRefItem` pick under their own `settings.pick_id`** - a reference item's own pick id was ignored and the id baked into the upload was used instead, so two references to one stored cloud or set were indistinguishable and a reference to something uploaded without an id could not be picked at all. The reference's id is now what the pick reports; set it on the reference, not on the item you upload.
