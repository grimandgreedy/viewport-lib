...
- **`DeviceResources::check_texture_slot` and `builtin_colourmap_id_checked` are public** - the first reports a texture bound into a slot that wants the other colour space, which an item type binding a consumer's texture should call and cannot implement itself (the mismatch log is the renderer's). The second is the non-panicking form of `builtin_colourmap_id`, for an upload resolving a default colourmap before the first frame has initialised them.
