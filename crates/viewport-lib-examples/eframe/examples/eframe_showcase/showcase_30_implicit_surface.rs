...
impl App {
    ...
    pub(crate) fn implicit_scene_items(&self) -> Vec<SceneRenderItem> {
        ...
        // Near (blue): in front of the blob cloud.
        {
            ...
            item.mesh_id = self.is_state.mesh_id;
            item.model =
        ...
        if self.is_state.sdf_variant == IsSdfVariant::MarchingCubes {
            if let Some(mc_id) = self.is_state.mc_mesh_id {
                let mut item = SceneRenderItem::default();
                item.mesh_id = mc_id;
...
pub(crate) fn build(app: &mut crate::App, renderer: &mut vpl::ViewportRenderer) {
    app.build_implicit_scene(renderer);
}
...
pub(crate) struct ScImplicitSurface;

/// The registry's handle to this showcase.
pub(crate) static SHOWCASE: ScImplicitSurface = ScImplicitSurface;

impl crate::Showcase for ScImplicitSurface {
