...
pub(crate) fn build(app: &mut crate::App, renderer: &mut vpl::ViewportRenderer) {
    app.build_foreground_scene(renderer);
}
...
pub(crate) fn scene(
    ...
    let (items, bg_colour, lighting, scene_gen, sel_gen) = {
        let items = foreground_scene_items(app);
        (items, None, crate::App::foreground_lighting(), 0, 0)
    };
    crate::SceneContents {
        items,
        bg_colour,
...
pub(crate) struct ScForegroundPass;

/// The registry's handle to this showcase.
pub(crate) static SHOWCASE: ScForegroundPass = ScForegroundPass;

impl crate::Showcase for ScForegroundPass {
