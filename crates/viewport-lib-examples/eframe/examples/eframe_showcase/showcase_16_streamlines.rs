...
pub(crate) fn scene(
    ...
    let (items, bg_colour, lighting, scene_gen, sel_gen) = {
        let (items, lighting, sg, ss) =
            stream_collect_scene_items(app);
...
pub(crate) fn frame(
    ...
    submit_stream_items(app, &mut *fd);
}
...
pub(crate) struct ScStreamlines;

/// The registry's handle to this showcase.
pub(crate) static SHOWCASE: ScStreamlines = ScStreamlines;

impl crate::Showcase for ScStreamlines {
