...
use vpl::{
    ...
    ColourmapId, FrameData, LightingSettings, SceneRenderItem, ViewportRenderer, VolumeMeshData,
    ...
    pub(crate) fn vm_lighting() -> LightingSettings {
        {
            ...
            _t.ground_colour = [0.3, 0.3, 0.4].into();
            _t
        }
    }
}
...
pub(crate) fn scene(
    ...
    let (items, bg_colour, lighting, scene_gen, sel_gen) = {
        let (items, lighting, sg, ss) = vm_collect_scene_items(app, frame);
...
pub(crate) struct ScVolumeMesh;

/// The registry's handle to this showcase.
pub(crate) static SHOWCASE: ScVolumeMesh = ScVolumeMesh;

impl crate::Showcase for ScVolumeMesh {
