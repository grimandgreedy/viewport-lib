...
pub(crate) fn scene(
    ...
    let (items, bg_colour, lighting, scene_gen, sel_gen) = {
        let (items, lighting, sg) = sl_collect(app);
...
pub(crate) struct ScSceneLights;

/// The registry's handle to this showcase.
pub(crate) static SHOWCASE: ScSceneLights = ScSceneLights;

impl crate::Showcase for ScSceneLights {
