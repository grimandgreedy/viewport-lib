...
pub(crate) fn tick(app: &mut crate::App, cx: &crate::ViewportCtx) {
    ...
    if app.pb_state.mode == vpl::RuntimeMode::Playback {
        let dt = cx.egui.input(|i| i.stable_dt.min(1.0 / 15.0));
        app.pb_state.time += dt;
...
pub(crate) struct ScPlaybackRuntime;

/// The registry's handle to this showcase.
pub(crate) static SHOWCASE: ScPlaybackRuntime = ScPlaybackRuntime;

impl crate::Showcase for ScPlaybackRuntime {
