...
pub(crate) fn scene(
    ...
    let (items, bg_colour, lighting, scene_gen, sel_gen) = {
        let items = if app.decal46_state.built {
            decal46_scene_items(app)
        } else {
            Vec::new()
        };
        let lighting = {
...
pub(crate) struct ScDecals;

/// The registry's handle to this showcase.
pub(crate) static SHOWCASE: ScDecals = ScDecals;

impl crate::Showcase for ScDecals {
