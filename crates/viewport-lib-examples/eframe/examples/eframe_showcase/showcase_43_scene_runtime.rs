...
pub(crate) fn scene(
    ...
    let (items, bg_colour, lighting, scene_gen, sel_gen) = {
        let items = if app.rt_state.built {
            rt_demo_scene_items(app)
        } else {
            Vec::new()
        };
        let lighting = {
...
pub(crate) struct ScSceneRuntime;

/// The registry's handle to this showcase.
pub(crate) static SHOWCASE: ScSceneRuntime = ScSceneRuntime;

impl crate::Showcase for ScSceneRuntime {
