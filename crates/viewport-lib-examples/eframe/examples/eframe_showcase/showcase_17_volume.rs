...
impl Default for VolumeState {
    fn default() -> Self {
        let mut iso_material = Material::from_colour(vpl::Colour::srgb(0.16, 0.4, 0.8, 1.0));
...
pub(crate) fn build(app: &mut crate::App, renderer: &mut vpl::ViewportRenderer) {
    app.build_volume_scene(renderer);
    app.ensure_surface_slice_mesh(renderer);
...
pub(crate) fn scene(
    ...
    let (items, bg_colour, lighting, scene_gen, sel_gen) = {
        let (items, lighting, sg, ss) = vol_collect_scene_items(app);
...
pub(crate) struct ScVolume;

/// The registry's handle to this showcase.
pub(crate) static SHOWCASE: ScVolume = ScVolume;

impl crate::Showcase for ScVolume {
