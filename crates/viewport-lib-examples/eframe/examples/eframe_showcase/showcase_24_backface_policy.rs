...
pub(crate) fn scene(
    ...
    // The host assigns `fd.effects.clip.objects` from these straight after the
    // scene hook runs, so the clip objects have to be contributed here rather
    // than from `frame`.
    out.clip_objects.extend(app.sa_clip_objects());

    let (items, bg_colour, lighting, scene_gen, sel_gen) = {
        let items = app.sa_scene_items();
        let sg = app.sa_state.scene.version();
        (items, None, crate::App::sa_lighting(), sg, 0)
    };
    crate::SceneContents {
        items,
        bg_colour,
...
pub(crate) fn overlay(app: &mut crate::App, ui: &mut crate::eframe::egui::Ui, cx: &crate::ViewportCtx) {
    ...
    app.draw_sa_labels(ui, cx.rect);
    app.draw_sa_row_labels(ui, cx.rect);
}
...
pub(crate) struct ScBackfacePolicy;

/// The registry's handle to this showcase.
pub(crate) static SHOWCASE: ScBackfacePolicy = ScBackfacePolicy;

impl crate::Showcase for ScBackfacePolicy {
    ...
    fn controls(&self, app: &mut crate::App, ui: &mut crate::eframe::egui::Ui, _frame: &crate::eframe::Frame) {
        controls_surface_appearance(app, ui)
    }
}
