...
pub(crate) fn scene(
    ...
    let (items, bg_colour, lighting, scene_gen, sel_gen) = {
        let (items, lighting, sg, ss) = iso_collect_scene_items(app);
...
pub(crate) struct ScIsolines;

/// The registry's handle to this showcase.
pub(crate) static SHOWCASE: ScIsolines = ScIsolines;

impl crate::Showcase for ScIsolines {
