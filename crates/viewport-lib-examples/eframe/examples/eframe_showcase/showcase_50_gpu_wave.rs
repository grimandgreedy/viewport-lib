...
pub(crate) fn scene(
    ...
    let (items, bg_colour, lighting, scene_gen, sel_gen) = {
        let (items, lighting) = wave_collect(app);
...
pub(crate) fn frame(app: &mut crate::App, fd: &mut vpl::FrameData, ctx: &crate::FrameCtx) {
    if app.wave_state.built {
        ...
        if let Some(renderer) = guard.callback_resources.get_mut::<vpl::ViewportRenderer>() {
            submit_wave_items(app, &mut *fd, renderer);
        }
    }
}
...
pub(crate) fn tick(app: &mut crate::App, cx: &crate::ViewportCtx) {
    ...
    if app.wave_state.built && !app.wave_state.paused {
...
pub(crate) struct ScGpuWave;

/// The registry's handle to this showcase.
pub(crate) static SHOWCASE: ScGpuWave = ScGpuWave;

impl crate::Showcase for ScGpuWave {
