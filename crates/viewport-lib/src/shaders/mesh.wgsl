...
// A transformed UV plus its screen-space derivatives, for textureSampleGrad.
struct SlotUv {
    uv: vec2<f32>,
    ddx: vec2<f32>,
    ddy: vec2<f32>,
...
// Apply a material texture slot's UV transform:
//   uv' = rotate((mesh_uv * scale + offset) - 0.5, rotation) + 0.5
