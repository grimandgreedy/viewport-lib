...
// The deformer descriptor vocabulary (`DeformStage`, `DeformerDesc`) lives in
// `viewport-lib-types` so a crate can define a deformer without the renderer;
// re-exported here so the registration machinery and public paths are unchanged.
pub use viewport_lib_types::plugin::deformer::{DeformStage, DeformerDesc};
