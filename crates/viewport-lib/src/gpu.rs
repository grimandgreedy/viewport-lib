...
/// bound array so freed texture slots can be left unbound. Named the same on
/// every wgpu leg.
///
/// Support is a property of the adapter, not of the backend: ask the adapter
/// rather than assuming from the platform. Apple silicon reports all three
/// through Metal argument buffers, so a Mac takes the bindless path, and WebGPU
/// does not. `recommended_device_features` requests these when the adapter has
/// them, which is what selects
