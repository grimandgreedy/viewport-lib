...
use viewport_lib::raytrace::{RtCamera, RtLight, RtMaterial, RtScene, RtSettings, Tracer};
...
struct AppState {
    ...
    // The tracer holds the compiled pipeline and uploaded scene, so re-tracing
    // on every camera move only re-dispatches instead of rebuilding everything.
    tracer: Tracer,
