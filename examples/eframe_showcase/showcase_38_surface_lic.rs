...
fn safe_normalize(v: [f32; 3]) -> [f32; 3] {
    ...
    if len < 1e-6 { [0.0; 3] } else { [v[0]/len, v[1]/len, v[2]/len] }
}
...
fn flow_torus_poloidal(theta: f32, phi: f32) -> [f32; 3] {
    safe_normalize([-phi.sin()*theta.cos(), phi.cos(), -phi.sin()*theta.sin()])
}
...
fn build_torus(ms: usize, ns: usize, rm: f32, rn: f32)
    -> (Vec<[f32;3]>, Vec<[f32;3]>, Vec<u32>, Vec<f32>, Vec<f32>)
{
    let mut pos = Vec::new(); let mut nrm = Vec::new();
    let mut ths = Vec::new(); let mut phs = Vec::new();
    for j in 0..ms {
    ...
    for j in 0..ms {
        ...
        for i in 0..ns {
            let ni = (i+1)%ns;
            let a=(j*ns+i) as u32; let b=(j*ns+ni) as u32;
            let c=(jn*ns+ni) as u32; let d=(jn*ns+i) as u32;
...
fn build_bumpy(segs: usize, size: f32, amp: f32, freq: f32)
    -> (Vec<[f32;3]>, Vec<[f32;3]>, Vec<u32>)
{
    ...
    let mut idx = Vec::new();
    for iz in 0..segs { for ix in 0..segs {
        let a=(iz*n+ix) as u32; let b=(iz*n+ix+1) as u32;
        let c=((iz+1)*n+ix+1) as u32; let d=((iz+1)*n+ix) as u32;
        idx.extend_from_slice(&[a,b,c,a,c,d]);
    }}
    (pos, nrm, idx)
}

fn build_sphere(lon: usize, lat: usize, r: f32)
    -> (Vec<[f32;3]>, Vec<[f32;3]>, Vec<u32>)
{
    let mut pos = Vec::new(); let mut nrm = Vec::new();
    let mut idx = Vec::new();
    for la in 0..=lat {
        ...
        for lo in 0..=lon {
            ...
            let (sp, cp) = (ph.sin(), ph.cos());
            let x = st*cp; let y = ct; let z = st*sp;
    ...
    for la in 0..lat {
        for lo in 0..lon {
            let a=(la*s+lo) as u32; let b=((la+1)*s+lo) as u32;
            let c=((la+1)*s+lo+1) as u32; let d=(la*s+lo+1) as u32;
            if la > 0       { idx.extend_from_slice(&[a,d,b]); }
            if la < lat-1   { idx.extend_from_slice(&[b,d,c]); }
        }
    }
    (pos, nrm, idx)
}
...
// Mid-tone colors (linear 0..1) so LIC can both darken and brighten visibly.
// White surfaces clip brightening; these sit around 0.4-0.5 for symmetric contrast.
const ROW_COLORS: [[f32; 3]; 3] = [
...
fn make_material(row: usize) -> Material {
    let mut m = Material::from_color(ROW_COLORS[row]);
...
fn upload_mesh(
    ...
    pos: Vec<[f32;3]>, nrm: Vec<[f32;3]>, idx: Vec<u32>,
    flow: Vec<[f32;3]>,
    col: usize, row: usize,
) -> MeshId {
...
pub(crate) fn build_lic_scene(app: &mut App, renderer: &mut ViewportRenderer) {
    ...
    for col in 0..3usize {
        // Row 0: Tori -- toroidal, poloidal, helical.
        {
            ...
            let flow: Vec<_> = ths.iter().zip(phs.iter()).map(|(&t, &p)| match col {
        ...
        // Row 1: Bumpy terrain -- topographic, uniform wind, vortex.
        {
            ...
            let flow: Vec<_> = pos.iter().zip(nrm.iter()).map(|(&p, &n)| match col {
...
pub(crate) fn controls_lic(app: &mut App, ui: &mut egui::Ui) {
    ...
    egui::Grid::new("lic_labels").num_columns(4).spacing([6.0, 4.0]).show(ui, |ui| {
        ui.label("");            ui.strong("Col 0");         ui.strong("Col 1");    ui.strong("Col 2");    ui.end_row();
        ui.strong("Tori");      ui.label("toroidal");       ui.label("poloidal");  ui.label("helical");   ui.end_row();
        ui.strong("Terrain");   ui.label("topographic");    ui.label("wind (+X)"); ui.label("vortex");    ui.end_row();
        ui.strong("Spheres");   ui.label("latitude");       ui.label("meridian");  ui.label("diagonal");  ui.end_row();
    });

    ui.separator();
    ui.label("Topographic flow (terrain col 0) shows where runoff converges\n\
