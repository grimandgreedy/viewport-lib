...
impl App {
    ...
    pub(crate) fn build_param_vis_scene(&mut self, renderer: &mut ViewportRenderer) {
        ...
        add_row(
            &mut self.param_vis_scene, renderer,
            &torus_data,  Z_TORUS,  IDX_TORUS,  &mut self.param_vis_node_ids,
            [0.55, 0.70, 0.65], false,
        );
        add_row(
            &mut self.param_vis_scene, renderer,
            &sphere_data, Z_SPHERE, IDX_SPHERE, &mut self.param_vis_node_ids,
            [0.7, 0.7, 0.7], false,
        );
        add_row(
            &mut self.param_vis_scene, renderer,
            &cube_data,   Z_CUBE,   IDX_CUBE,   &mut self.param_vis_node_ids,
            [0.72, 0.65, 0.55], false,
        );
        add_row(
            &mut self.param_vis_scene, renderer,
            &plane_data,  Z_PLANE,  IDX_PLANE,  &mut self.param_vis_node_ids,
            [0.65, 0.65, 0.80], true,
        );
            ...
            for (col, (mode, _)) in MODES.iter().enumerate() {
                let mat = {
                    ...
                    m.param_vis = if self.param_vis_on {
                        Some(ParamVis { mode: *mode, scale: self.param_vis_scale })
                    } else {
                        None
                    };
                    m.two_sided = two_sided;
