...
pub(crate) fn build_warp_scene(app: &mut App, renderer: &mut ViewportRenderer) {
    ...
    let plane_id = {
        ...
        let disp: Vec<[f32; 3]> = mesh.positions.iter().map(|&[x, _, z]| {
            let freq = std::f32::consts::TAU / 5.0;
            let h = (freq * x).sin() * (freq * z).cos();
            [0.0, h, 0.0]
        }).collect();
        mesh.attributes.insert("warp".to_string(), AttributeData::VertexVector(disp));
        renderer.resources_mut().upload_mesh_data(&app.device, &mesh).expect("warp plane")
    };

    // Centre: sphere with 4 azimuthal lobes.
    let sphere_4lobe_id = {
        let mut mesh = primitives::sphere(1.5, 64, 32);
        let disp: Vec<[f32; 3]> = mesh.positions.iter().zip(mesh.normals.iter()).map(|(&[x, y, _z], &nrm)| {
            let phi = y.atan2(x); // azimuth around Z axis
            let amp = (4.0 * phi).sin();
            [nrm[0] * amp, nrm[1] * amp, nrm[2] * amp]
        }).collect();
        mesh.attributes.insert("warp".to_string(), AttributeData::VertexVector(disp));
        renderer.resources_mut().upload_mesh_data(&app.device, &mesh).expect("warp sphere 4lobe")
    };

    // Right: sphere with Y2,0 elongation mode.
    let sphere_y20_id = {
        let mut mesh = primitives::sphere(1.5, 64, 32);
        let disp: Vec<[f32; 3]> = mesh.positions.iter().zip(mesh.normals.iter()).map(|(&[x, y, z], &nrm)| {
            let r2 = x * x + y * y + z * z;
...
pub(crate) fn controls_warp(app: &mut App, ui: &mut egui::Ui) {
    ...
    ui.separator();
    ui.label("Displacement vectors are baked into each mesh once at load time.\n\
...
pub(crate) fn warp_scene_items(app: &App) -> Vec<SceneRenderItem> {
    ...
    ids.iter().zip(colors.iter()).zip(offsets.iter()).map(|((&mesh_id, &color), &tx)| {
        let mut item = SceneRenderItem::default();
        item.mesh_id = mesh_id;
        item.model = glam::Mat4::from_translation(glam::Vec3::new(tx, 0.0, 0.0)).to_cols_array_2d();
...
pub(crate) fn warp_lighting() -> LightingSettings {
    ...
    LightingSettings {
        lights: vec![
            LightSource {
                kind: LightKind::Directional { direction: [0.3, 0.8, 0.5] },
                color: [1.0, 1.0, 1.0],
                intensity: 0.7,
            },
            LightSource {
                kind: LightKind::Directional { direction: [-0.3, -0.5, -0.5] },
                color: [0.8, 0.85, 1.0],
                intensity: 0.3,
            },
        ],
