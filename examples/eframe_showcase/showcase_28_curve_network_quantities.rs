...
pub(crate) fn controls_cnq(app: &mut App, ui: &mut egui::Ui) {
    ...
    for (mode, label) in [
        ...
        if ui.radio(app.cnq_state.mode == mode, label).clicked() {
            app.cnq_state.mode = mode;
        }
    }

    ui.separator();
    ui.label("Base line width:");
    ui.add(egui::Slider::new(&mut app.cnq_state.line_width, 1.0..=10.0).step_by(0.5));
}
...
pub(crate) fn make_cnq_polyline_item(app: &App) -> PolylineItem {
    ...
    item.line_width = app.cnq_state.line_width;

    match app.cnq_state.mode {
        CnqMode::EdgeScalar => {
        ...
        CnqMode::NodeRadius => {
            ...
            item.node_radii = (0..n)
                .map(|k| {
                    let t = k as f32 / (n - 1) as f32;
                    let base = app.cnq_state.line_width;
