...
fn probe_example_scene() {
    ...
    let ground = renderer.resources_mut()
        .upload_mesh_data(&device, &viewport_lib::primitives::cuboid(span + 8.0, 12.0, 0.4)).unwrap();
    let sphere = renderer.resources_mut()
        .upload_mesh_data(&device, &viewport_lib::primitives::sphere(0.8, 40, 20)).unwrap();

    let mut items = Vec::new();
    let mut g = SceneRenderItem::default();
    g.mesh_id = ground;
    g.model = glam::Mat4::from_translation(glam::Vec3::new(span * 0.5, 0.0, -0.2)).to_cols_array_2d();
    ...
    for c in 0..cols {
        ...
        let mut s = SceneRenderItem::default();
        s.mesh_id = sphere;
        s.model = glam::Mat4::from_translation(glam::Vec3::new(c as f32 * spacing, 0.8, 0.8)).to_cols_array_2d();
    ...
    let mean_luma = |px: &[u8]| {
        ...
        for p in px.chunks_exact(4) { s += p[0] as f32 + p[1] as f32 + p[2] as f32; n += 3.0; }
        s / n
    };
    let make = |orient: glam::Quat, dist: f32, hemi: f32| {
        let cam = Camera { center: glam::Vec3::new(span * 0.5, 0.4, 0.6), distance: dist, orientation: orient, ..Camera::default() };
        let mut frame = FrameData::default();
        frame.camera.render_camera = { let mut rc = RenderCamera::from_camera(&cam); rc.aspect = 1.0; rc };
        frame.camera.viewport_size = [size as f32, size as f32];
        frame.viewport.show_grid = false; frame.viewport.show_axes_indicator = false;
        frame.viewport.background_colour = Some([0.12, 0.12, 0.14, 1.0]);
        let mut light = LightSource::default();
        light.cast_shadows = true;
        light.kind = LightKind::Directional { direction: [0.4, 0.5, 0.9] };
        light.intensity = 4.0;
        frame.effects.lighting.lights = vec![light];
    ...
    for (label, orient, dist, hemi) in [
        ...
        eprintln!("{label:11} | {hemi:.2} | {:10.3} | {:.5} | {:.1}", st.current_ev, st.exposure, mean_luma(&px));
    }
}
